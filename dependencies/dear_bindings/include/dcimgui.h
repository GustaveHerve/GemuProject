...
// CIMGUI_API is used for core imgui functions, CIMGUI_IMPL_API is used for the default backends files
// (imgui_impl_xxx.h) Using dear imgui via a shared library is not recommended: we don't guarantee backward nor forward
// ABI compatibility + this is a call-heavy library and function call overhead adds up.
#ifndef CIMGUI_API
#define CIMGUI_API
...
#define CIMGUI_CHECKVERSION()                                                                                          \
    ImGui_DebugCheckVersionAndDataLayout(IMGUI_VERSION,                                                                \
                                         sizeof(ImGuiIO),                                                              \
                                         sizeof(ImGuiStyle),                                                           \
                                         sizeof(ImVec2),                                                               \
                                         sizeof(ImVec4),                                                               \
...
// - To use 32-bit indices: override with '#define ImDrawIdx unsigned int' in your imconfig.h file.
    ...
    typedef struct ImDrawChannel_t ImDrawChannel; // Temporary storage to output draw commands out of order, used by
                                                  // ImDrawListSplitter and ImDrawList::ChannelsSplit()
    typedef struct ImDrawCmd_t ImDrawCmd;   // A single draw command within a parent ImDrawList (generally maps to 1 GPU
                                            // draw call, unless it is a callback)
    typedef struct ImDrawData_t ImDrawData; // All draw command lists required to render the frame + pos/size
                                            // coordinates to use for the projection matrix.
    typedef struct ImDrawList_t ImDrawList; // A single draw command list (generally one per window, conceptually you
                                            // may see this as a dynamic "mesh" builder)
    typedef struct ImDrawListSharedData_t
        ImDrawListSharedData; // Data shared among multiple draw lists (typically owned by parent ImGui context, but you
                              // may create one yourself)
    typedef struct ImDrawListSplitter_t ImDrawListSplitter; // Helper to split a draw list into different layers which
                                                            // can be drawn into out of order, then flattened back.
    typedef struct ImDrawVert_t ImDrawVert; // A single vertex (pos + uv + col = 20 bytes by default. Override layout
    ...
    typedef struct ImFontAtlasRect_t
    ...
    typedef struct ImColor_t ImColor; // Helper functions to create a color that can be converted to either u32 or
    ...
    typedef struct ImGuiInputTextCallbackData_t
        ImGuiInputTextCallbackData;             // Shared state of InputText() when using custom ImGuiInputTextCallback
                                                // (rare/advanced use)
    ...
    typedef struct ImGuiPlatformIO_t
        ImGuiPlatformIO; // Interface between platform/renderer backends and ImGui (e.g. Clipboard, IME hooks). Extends
    ...
    typedef struct ImGuiSelectionExternalStorage_t
        ImGuiSelectionExternalStorage; // Optional helper to apply multi-selection requests to existing randomly
                                       // accessible storage.
    ...
    typedef struct ImGuiTableSortSpecs_t ImGuiTableSortSpecs; // Sorting specifications for a table (often handling sort
    ...
    typedef struct ImGuiViewport_t ImGuiViewport; // A Platform Window (always only one in 'master' branch), in the
    ...
    typedef int ImGuiMouseSource; // -> enum ImGuiMouseSource      // Enum; A mouse input source identifier (Mouse,
                                  // TouchScreen, Pen)
    typedef ImU8
...
#endif // ImWchar [configurable type: override in imconfig.h with '#define IMGUI_USE_WCHAR32' to support Unicode planes
       // 1-16]
...
#define ImTextureID_Invalid ((ImTextureID)0)
    ...
    //      inline ImTextureRef ImTextureRefFromID(ImTextureID tex_id) { ImTextureRef tex_ref = { ._TexData = NULL,
    ...
    CIMGUI_API ImTextureID ImTextureRef_GetTexID(
        const ImTextureRef *self); // == (_TexData ? _TexData->TexID : _TexID) // Implemented below in the file.
    IM_MSVC_RUNTIME_CHECKS_RESTORE
    ...
    // Main
    CIMGUI_API ImGuiIO *ImGui_GetIO(void); // access the ImGuiIO structure (mouse/keyboard/gamepad inputs, time, various
                                           // configuration options/flags)
    CIMGUI_API ImGuiPlatformIO *
    ImGui_GetPlatformIO(void); // access the ImGuiPlatformIO structure (mostly hooks/functions to connect to
                               // platform/renderer and OS Clipboard, IME etc.)
    CIMGUI_API ImGuiStyle *ImGui_GetStyle(void); // access the Style structure (colors, sizes). Always use
    ...
    CIMGUI_API void ImGui_EndFrame(
        void); // ends the Dear ImGui frame. automatically called by Render(). If you don't need to render data
    ...
    CIMGUI_API void
    ImGui_Render(void); // ends the Dear ImGui frame, finalize the draw data. You can then get call GetDrawData().
    CIMGUI_API ImDrawData *
    ImGui_GetDrawData(void); // valid after Render() and until the next call to NewFrame(). Call
    ...
    CIMGUI_API void
    ImGui_ShowMetricsWindow(bool *p_open /* = NULL */); // create Metrics/Debugger window. display Dear ImGui internals:
    ...
    CIMGUI_API void
    ImGui_ShowIDStackToolWindowEx(bool *p_open /* = NULL */); // create Stack Tool window. hover items with mouse to
                                                              // query information about the source of their unique ID.
    CIMGUI_API void ImGui_ShowAboutWindow(bool *p_open /* = NULL */); // create About window. display Dear ImGui
                                                                      // version, credits and build/system information.
    CIMGUI_API void ImGui_ShowStyleEditor(
        ImGuiStyle
            *ref /* = NULL */); // add style editor block (not a window). you can pass in a reference ImGuiStyle
    ...
    CIMGUI_API void ImGui_ShowUserGuide(void); // add basic help/info block (not a window): how to manipulate ImGui as
                                               // an end-user (mouse/keyboard controls).
    CIMGUI_API const char *ImGui_GetVersion(void); // get the compiled version string e.g. "1.80 WIP" (essentially the
    ...
    CIMGUI_API void
    ImGui_StyleColorsLight(ImGuiStyle *dst /* = NULL */); // best used with borders and a custom, thicker font
    ...
    CIMGUI_API bool ImGui_BeginChildID(ImGuiID id,
                                       ImVec2 size /* = ImVec2(0, 0) */,
                                       ImGuiChildFlags child_flags /* = 0 */,
                                       ImGuiWindowFlags window_flags /* = 0 */);
    ...
    CIMGUI_API bool
    ImGui_IsWindowFocused(ImGuiFocusedFlags flags /* = 0 */); // is current window focused? or its root/child, depending
                                                              // on flags. see flags for options.
    CIMGUI_API bool ImGui_IsWindowHovered(
        ImGuiHoveredFlags
            flags /* = 0 */); // is current window hovered and hoverable (e.g. not blocked by a popup/modal)? See
    ...
    CIMGUI_API ImVec2
    ImGui_GetWindowPos(void); // get current window position in screen space (IT IS UNLIKELY YOU EVER NEED TO USE THIS.
                              // Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)
    CIMGUI_API ImVec2
    ImGui_GetWindowSize(void); // get current window size (IT IS UNLIKELY YOU EVER NEED TO USE THIS. Consider always
    ...
    CIMGUI_API void
    ImGui_SetNextWindowPosEx(ImVec2 pos,
                             ImGuiCond cond /* = 0 */,
                             ImVec2 pivot /* = ImVec2(0, 0) */); // set next window position. call before Begin(). use
                                                                 // pivot=(0.5f,0.5f) to center on given point, etc.
    CIMGUI_API void
    ImGui_SetNextWindowSize(ImVec2 size, ImGuiCond cond /* = 0 */); // set next window size. set axis to 0.0f to force
    ...
    CIMGUI_API void ImGui_SetNextWindowSizeConstraints(
        ...
        void *custom_callback_data /* = NULL */); // set next window size limits. use 0.0f or FLT_MAX if you don't want
    ...
    CIMGUI_API void ImGui_SetNextWindowContentSize(
        ImVec2 size); // set next window content size (~ scrollable client area, which enforce the range of scrollbars).
    ...
    CIMGUI_API void
    ImGui_SetNextWindowCollapsed(bool collapsed,
    ...
    CIMGUI_API void ImGui_SetNextWindowScroll(
        ImVec2 scroll); // set next window scrolling value (use < 0.0f to not affect a given axis).
    ...
    CIMGUI_API void
    ImGui_SetWindowCollapsed(bool collapsed,
    ...
    CIMGUI_API void
    ImGui_SetWindowPosStr(const char *name, ImVec2 pos, ImGuiCond cond /* = 0 */); // set named window position.
    ...
    CIMGUI_API void ImGui_SetWindowCollapsedStr(const char *name,
                                                bool collapsed,
                                                ImGuiCond cond /* = 0 */); // set named window collapsed state
    CIMGUI_API void
    ImGui_SetWindowFocusStr(const char *name); // set named window to be focused / top-most. use NULL to remove focus.
    ...
    CIMGUI_API float
    ImGui_GetScrollMaxX(void); // get maximum scrolling amount ~~ ContentSize.x - WindowSize.x - DecorationsSize.x
    CIMGUI_API float
    ImGui_GetScrollMaxY(void); // get maximum scrolling amount ~~ ContentSize.y - WindowSize.y - DecorationsSize.y
    CIMGUI_API void
    ImGui_SetScrollHereX(float center_x_ratio /* = 0.5f */); // adjust scrolling amount to make current cursor position
    ...
    CIMGUI_API void
    ImGui_SetScrollHereY(float center_y_ratio /* = 0.5f */); // adjust scrolling amount to make current cursor position
    ...
    //  applied == GLOBAL SCALING FACTORS WILL APPLY TWICE!
    CIMGUI_API void ImGui_PushFontFloat(
        ImFont *font,
    ...
    CIMGUI_API float
    ImGui_GetFontSize(void); // get current scaled font size (= height in pixels). AFTER global scale factors applied.
                             // *IMPORTANT* DO NOT PASS THIS VALUE TO PushFont()! Use ImGui::GetStyle().FontSizeBase to
                             // get value before global scale factors.
    CIMGUI_API ImFontBaked *
    ImGui_GetFontBaked(void); // get current font bound at current size // == GetFont()->GetFontBaked(GetFontSize())
    ...
    CIMGUI_API void ImGui_PushStyleVarX(ImGuiStyleVar idx,
                                        float val_x); // modify X component of a style ImVec2 variable. "
    CIMGUI_API void ImGui_PushStyleVarY(ImGuiStyleVar idx,
    ...
    CIMGUI_API void ImGui_SetNextItemWidth(
        float item_width); // set width of the _next_ common large "item+label" widget. >0.0f: width in pixels, <0.0f
                           // align xx pixels to the right of window (so -FLT_MIN always align width to the right side)
    CIMGUI_API float ImGui_CalcItemWidth(void); // width of item given pushed settings and current cursor position. NOT
                                                // necessarily the width of last item unlike most 'Item' functions.
    CIMGUI_API void ImGui_PushTextWrapPos(
        float wrap_local_pos_x /* = 0.0f */); // push word-wrapping position for Text*() commands. < 0.0f: no wrapping;
                                              // 0.0f: wrap to end of window (or column); > 0.0f: wrap at 'wrap_pos_x'
    ...
    CIMGUI_API ImVec2 ImGui_GetFontTexUvWhitePixel(
        void); // get UV coordinate for a white pixel, useful to draw custom shapes via the ImDrawList API
    ...
    CIMGUI_API ImU32 ImGui_GetColorU32ImVec4(
        ImVec4 col); // retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
    CIMGUI_API ImU32 ImGui_GetColorU32ImU32(ImU32 col); // Implied alpha_mul = 1.0f
    CIMGUI_API ImU32 ImGui_GetColorU32ImU32Ex(
        ImU32 col, float alpha_mul /* = 1.0f */); // retrieve given color with style alpha applied, packed as a 32-bit
    ...
    CIMGUI_API ImVec2
    ImGui_GetCursorScreenPos(void); // cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND (prefer using
                                    // this rather than GetCursorPos(), also more useful to work with ImDrawList API).
    CIMGUI_API void
    ImGui_SetCursorScreenPos(ImVec2 pos); // cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND.
    CIMGUI_API ImVec2
    ImGui_GetContentRegionAvail(void); // available space from current position. THIS IS YOUR BEST FRIEND.
    ...
    CIMGUI_API ImVec2
    ImGui_GetCursorStartPos(void); // [window-local] initial cursor position, in window-local coordinates. Call
                                   // GetCursorScreenPos() after Begin() to get the absolute coordinates version.

    // Other layout functions
    CIMGUI_API void ImGui_Separator(void); // separator, generally horizontal. inside a menu bar or in horizontal layout
                                           // mode, this becomes a vertical separator.
    CIMGUI_API void ImGui_SameLine(void);  // Implied offset_from_start_x = 0.0f, spacing = -1.0f
    CIMGUI_API void
    ImGui_SameLineEx(float offset_from_start_x /* = 0.0f */,
    ...
    CIMGUI_API void ImGui_Dummy(ImVec2 size); // add a dummy item of given size. unlike InvisibleButton(), Dummy() won't
                                              // take the mouse click or be navigable into.
    CIMGUI_API void ImGui_Indent(void);       // Implied indent_w = 0.0f
    CIMGUI_API void ImGui_IndentEx(float indent_w /* = 0.0f */); // move content position toward the right, by indent_w,
                                                                 // or style.IndentSpacing if indent_w <= 0
    CIMGUI_API void ImGui_Unindent(void);                        // Implied indent_w = 0.0f
    CIMGUI_API void ImGui_UnindentEx(float indent_w /* = 0.0f */); // move content position back to the left, by
    ...
    CIMGUI_API void ImGui_EndGroup(
        void); // unlock horizontal starting position + capture the whole group bounding box into one "item" (so you can
               // use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)
    CIMGUI_API void ImGui_AlignTextToFramePadding(
        void); // vertically align upcoming text baseline to FramePadding.y so that it will align properly to regularly
    ...
    CIMGUI_API float
    ImGui_GetFrameHeightWithSpacing(void); // ~ FontSize + style.FramePadding.y * 2 + style.ItemSpacing.y (distance in
    ...
    CIMGUI_API void ImGui_PushIDStr(const char *str_id_begin,
    ...
    CIMGUI_API ImGuiID
    ImGui_GetID(const char *str_id); // calculate unique ID (hash of whole ID stack + given parameter). e.g. if you want
    ...
    CIMGUI_API void ImGui_TextWrapped(const char *fmt, ...)
    ...
    CIMGUI_API bool
    ImGui_SmallButton(const char *label); // button with (FramePadding.y == 0) to easily embed within text
    CIMGUI_API bool
    ImGui_InvisibleButton(const char *str_id,
                          ImVec2 size,
                          ImGuiButtonFlags flags /* = 0 */); // flexible button behavior without the visuals, frequently
    ...
    CIMGUI_API void
    ImGui_ProgressBar(float fraction, ImVec2 size_arg /* = ImVec2(-FLT_MIN, 0) */, const char *overlay /* = NULL */);
    CIMGUI_API void ImGui_Bullet(void); // draw a small circle + keep the cursor on the same line. advance cursor x
    ...
    CIMGUI_API bool ImGui_TextLinkOpenURLEx(
        const char *label,
    ...
    CIMGUI_API void ImGui_Image(ImTextureRef tex_ref,
                                ImVec2 image_size); // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1)
    CIMGUI_API void ImGui_ImageEx(ImTextureRef tex_ref,
                                  ImVec2 image_size,
                                  ImVec2 uv0 /* = ImVec2(0, 0) */,
                                  ImVec2 uv1 /* = ImVec2(1, 1) */);
    CIMGUI_API void ImGui_ImageWithBg(ImTextureRef tex_ref,
                                      ImVec2 image_size); // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), bg_col =
                                                          // ImVec4(0, 0, 0, 0), tint_col = ImVec4(1, 1, 1, 1)
    CIMGUI_API void ImGui_ImageWithBgEx(ImTextureRef tex_ref,
                                        ImVec2 image_size,
                                        ImVec2 uv0 /* = ImVec2(0, 0) */,
                                        ImVec2 uv1 /* = ImVec2(1, 1) */,
                                        ImVec4 bg_col /* = ImVec4(0, 0, 0, 0) */,
                                        ImVec4 tint_col /* = ImVec4(1, 1, 1, 1) */);
    CIMGUI_API bool ImGui_ImageButton(const char *str_id,
    ...
    ImGui_DragFloat(const char *label,
                    float *v); // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
    CIMGUI_API bool ImGui_DragFloatEx(const char *label,
                                      float *v,
                                      float v_speed /* = 1.0f */,
                                      float v_min /* = 0.0f */,
                                      float v_max /* = 0.0f */,
    ...
    CIMGUI_API bool ImGui_DragFloat2Ex(const char *label,
                                       float v[2],
                                       float v_speed /* = 1.0f */,
                                       float v_min /* = 0.0f */,
                                       float v_max /* = 0.0f */,
    ...
    CIMGUI_API bool ImGui_DragFloat3Ex(const char *label,
                                       float v[3],
                                       float v_speed /* = 1.0f */,
                                       float v_min /* = 0.0f */,
                                       float v_max /* = 0.0f */,
    ...
    CIMGUI_API bool ImGui_DragFloat4Ex(const char *label,
                                       float v[4],
                                       float v_speed /* = 1.0f */,
                                       float v_min /* = 0.0f */,
                                       float v_max /* = 0.0f */,
                                       const char *format /* = "%.3f" */,
                                       ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool ImGui_DragFloatRange2(const char *label,
                                          float *v_current_min,
                                          float *v_current_max); // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f,
                                                                 // format = "%.3f", format_max = NULL, flags = 0
    CIMGUI_API bool ImGui_DragFloatRange2Ex(const char *label,
                                            float *v_current_min,
                                            float *v_current_max,
                                            float v_speed /* = 1.0f */,
                                            float v_min /* = 0.0f */,
    ...
    CIMGUI_API bool ImGui_DragIntEx(const char *label,
                                    int *v,
                                    float v_speed /* = 1.0f */,
                                    int v_min /* = 0 */,
                                    int v_max /* = 0 */,
    ...
    CIMGUI_API bool ImGui_DragInt2Ex(const char *label,
                                     int v[2],
                                     float v_speed /* = 1.0f */,
                                     int v_min /* = 0 */,
                                     int v_max /* = 0 */,
    ...
    CIMGUI_API bool ImGui_DragInt3Ex(const char *label,
                                     int v[3],
                                     float v_speed /* = 1.0f */,
                                     int v_min /* = 0 */,
                                     int v_max /* = 0 */,
    ...
    CIMGUI_API bool ImGui_DragInt4Ex(const char *label,
                                     int v[4],
                                     float v_speed /* = 1.0f */,
                                     int v_min /* = 0 */,
                                     int v_max /* = 0 */,
                                     const char *format /* = "%d" */,
                                     ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool ImGui_DragIntRange2(const char *label,
                                        int *v_current_min,
                                        int *v_current_max); // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format =
                                                             // "%d", format_max = NULL, flags = 0
    CIMGUI_API bool ImGui_DragIntRange2Ex(const char *label,
                                          int *v_current_min,
                                          int *v_current_max,
                                          float v_speed /* = 1.0f */,
                                          int v_min /* = 0 */,
                                          ...
                                          ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_DragScalar(const char *label,
                     ImGuiDataType data_type,
                     void *p_data); // Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
    CIMGUI_API bool ImGui_DragScalarEx(const char *label,
                                       ImGuiDataType data_type,
                                       void *p_data,
                                       float v_speed /* = 1.0f */,
                                       ...
                                       ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_DragScalarN(const char *label,
                      ImGuiDataType data_type,
                      void *p_data,
                      int components); // Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
    CIMGUI_API bool ImGui_DragScalarNEx(const char *label,
                                        ImGuiDataType data_type,
                                        void *p_data,
                                        int components,
                                        float v_speed /* = 1.0f */,
    ...
    //   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
    CIMGUI_API bool
    ImGui_SliderFloat(const char *label, float *v, float v_min, float v_max); // Implied format = "%.3f", flags = 0
    CIMGUI_API bool
    ImGui_SliderFloatEx(const char *label,
                        float *v,
                        float v_min,
                        float v_max,
                        const char *format /* = "%.3f" */,
    ...
    CIMGUI_API bool
    ImGui_SliderFloat2(const char *label, float v[2], float v_min, float v_max); // Implied format = "%.3f", flags = 0
    CIMGUI_API bool ImGui_SliderFloat2Ex(const char *label,
                                         float v[2],
                                         float v_min,
                                         float v_max,
                                         const char *format /* = "%.3f" */,
                                         ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_SliderFloat3(const char *label, float v[3], float v_min, float v_max); // Implied format = "%.3f", flags = 0
    CIMGUI_API bool ImGui_SliderFloat3Ex(const char *label,
                                         float v[3],
                                         float v_min,
                                         float v_max,
                                         const char *format /* = "%.3f" */,
                                         ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_SliderFloat4(const char *label, float v[4], float v_min, float v_max); // Implied format = "%.3f", flags = 0
    CIMGUI_API bool ImGui_SliderFloat4Ex(const char *label,
                                         float v[4],
                                         float v_min,
                                         float v_max,
                                         const char *format /* = "%.3f" */,
    ...
    CIMGUI_API bool ImGui_SliderAngle(
        const char *label,
        float *v_rad); // Implied v_degrees_min = -360.0f, v_degrees_max = +360.0f, format = "%.0f deg", flags = 0
    CIMGUI_API bool ImGui_SliderAngleEx(const char *label,
                                        float *v_rad,
                                        ...
                                        ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_SliderInt(const char *label, int *v, int v_min, int v_max); // Implied format = "%d", flags = 0
    CIMGUI_API bool ImGui_SliderIntEx(const char *label,
                                      int *v,
                                      int v_min,
                                      int v_max,
                                      const char *format /* = "%d" */,
                                      ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_SliderInt2(const char *label, int v[2], int v_min, int v_max); // Implied format = "%d", flags = 0
    CIMGUI_API bool ImGui_SliderInt2Ex(const char *label,
                                       int v[2],
                                       int v_min,
                                       int v_max,
                                       const char *format /* = "%d" */,
                                       ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_SliderInt3(const char *label, int v[3], int v_min, int v_max); // Implied format = "%d", flags = 0
    CIMGUI_API bool ImGui_SliderInt3Ex(const char *label,
                                       int v[3],
                                       int v_min,
                                       int v_max,
                                       const char *format /* = "%d" */,
                                       ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_SliderInt4(const char *label, int v[4], int v_min, int v_max); // Implied format = "%d", flags = 0
    CIMGUI_API bool ImGui_SliderInt4Ex(const char *label,
                                       int v[4],
                                       int v_min,
                                       int v_max,
                                       const char *format /* = "%d" */,
                                       ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool ImGui_SliderScalar(const char *label,
                                       ImGuiDataType data_type,
                                       void *p_data,
                                       const void *p_min,
                                       const void *p_max); // Implied format = NULL, flags = 0
    CIMGUI_API bool ImGui_SliderScalarEx(const char *label,
                                         ImGuiDataType data_type,
                                         void *p_data,
                                         const void *p_min,
                                         const void *p_max,
                                         const char *format /* = NULL */,
                                         ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool ImGui_SliderScalarN(const char *label,
                                        ImGuiDataType data_type,
                                        void *p_data,
                                        int components,
                                        const void *p_min,
    ...
    CIMGUI_API bool ImGui_VSliderFloat(
        const char *label, ImVec2 size, float *v, float v_min, float v_max); // Implied format = "%.3f", flags = 0
    CIMGUI_API bool ImGui_VSliderFloatEx(const char *label,
                                         ImVec2 size,
                                         float *v,
                                         float v_min,
                                         float v_max,
                                         ...
                                         ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_VSliderInt(const char *label, ImVec2 size, int *v, int v_min, int v_max); // Implied format = "%d", flags = 0
    CIMGUI_API bool ImGui_VSliderIntEx(const char *label,
                                       ImVec2 size,
                                       int *v,
                                       int v_min,
                                       int v_max,
                                       const char *format /* = "%d" */,
                                       ImGuiSliderFlags flags /* = 0 */);
    CIMGUI_API bool ImGui_VSliderScalar(const char *label,
                                        ImVec2 size,
                                        ImGuiDataType data_type,
                                        void *p_data,
                                        const void *p_min,
                                        const void *p_max); // Implied format = NULL, flags = 0
    CIMGUI_API bool ImGui_VSliderScalarEx(const char *label,
                                          ImVec2 size,
                                          ImGuiDataType data_type,
                                          void *p_data,
                                          const void *p_min,
    ...
    // InputDouble etc.
    CIMGUI_API bool ImGui_InputText(const char *label,
                                    char *buf,
                                    size_t buf_size,
                                    ImGuiInputTextFlags flags /* = 0 */); // Implied callback = NULL, user_data = NULL
    CIMGUI_API bool ImGui_InputTextEx(const char *label,
                                      char *buf,
                                      size_t buf_size,
    ...
    CIMGUI_API bool ImGui_InputTextMultiline(
        const char *label,
        char *buf,
        size_t buf_size); // Implied size = ImVec2(0, 0), flags = 0, callback = NULL, user_data = NULL
    CIMGUI_API bool ImGui_InputTextMultilineEx(const char *label,
                                               char *buf,
                                               size_t buf_size,
                                               ImVec2 size /* = ImVec2(0, 0) */,
                                               ImGuiInputTextFlags flags /* = 0 */,
                                               ...
                                               void *user_data /* = NULL */);
    CIMGUI_API bool
    ImGui_InputTextWithHint(const char *label,
                            const char *hint,
                            char *buf,
                            size_t buf_size,
                            ImGuiInputTextFlags flags /* = 0 */); // Implied callback = NULL, user_data = NULL
    CIMGUI_API bool ImGui_InputTextWithHintEx(const char *label,
                                              const char *hint,
                                              char *buf,
                                              size_t buf_size,
                                              ImGuiInputTextFlags flags /* = 0 */,
    ...
    CIMGUI_API bool ImGui_InputFloatEx(const char *label,
                                       float *v,
                                       float step /* = 0.0f */,
    ...
    CIMGUI_API bool ImGui_InputDoubleEx(const char *label,
                                        double *v,
                                        double step /* = 0.0 */,
                                        ...
                                        ImGuiInputTextFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_InputScalar(const char *label,
                      ImGuiDataType data_type,
                      void *p_data); // Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
    CIMGUI_API bool ImGui_InputScalarEx(const char *label,
                                        ImGuiDataType data_type,
                                        void *p_data,
                                        ...
                                        ImGuiInputTextFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_InputScalarN(const char *label,
                       ImGuiDataType data_type,
                       void *p_data,
                       int components); // Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
    CIMGUI_API bool ImGui_InputScalarNEx(const char *label,
                                         ImGuiDataType data_type,
                                         void *p_data,
                                         int components,
                                         const void *p_step /* = NULL */,
    ...
    CIMGUI_API bool ImGui_ColorPicker4(const char *label,
                                       float col[4],
                                       ImGuiColorEditFlags flags /* = 0 */,
                                       const float *ref_col /* = NULL */);
    CIMGUI_API bool ImGui_ColorButton(const char *desc_id,
                                      ImVec4 col,
                                      ImGuiColorEditFlags flags /* = 0 */); // Implied size = ImVec2(0, 0)
    CIMGUI_API bool ImGui_ColorButtonEx(const char *desc_id,
                                        ImVec4 col,
                                        ImGuiColorEditFlags flags /* = 0 */,
    ...
    CIMGUI_API bool ImGui_TreeNodeExStr(const char *str_id, ImGuiTreeNodeFlags flags, const char *fmt, ...)
        IM_FMTARGS(3);
    CIMGUI_API bool ImGui_TreeNodeExPtr(const void *ptr_id, ImGuiTreeNodeFlags flags, const char *fmt, ...)
        IM_FMTARGS(3);
    CIMGUI_API bool ImGui_TreeNodeExV(const char *str_id, ImGuiTreeNodeFlags flags, const char *fmt, va_list args)
        IM_FMTLIST(3);
    CIMGUI_API bool ImGui_TreeNodeExVPtr(const void *ptr_id, ImGuiTreeNodeFlags flags, const char *fmt, va_list args)
        IM_FMTLIST(3);
    CIMGUI_API void
    ImGui_TreePush(const char *str_id); // ~ Indent()+PushID(). Already called by TreeNode() when returning true, but
    ...
    CIMGUI_API float
    ImGui_GetTreeNodeToLabelSpacing(void); // horizontal distance preceding label when using TreeNode*() or Bullet() ==
    ...
    CIMGUI_API void ImGui_SetNextItemOpen(bool is_open,
    ...
    CIMGUI_API bool ImGui_SelectableEx(
        ...
        ImVec2 size /* = ImVec2(0, 0) */); // "bool selected" carry the selection state (read-only). Selectable() is
    ...
    CIMGUI_API bool ImGui_SelectableBoolPtr(const char *label,
                                            bool *p_selected,
                                            ImGuiSelectableFlags flags /* = 0 */); // Implied size = ImVec2(0, 0)
    CIMGUI_API bool
    ImGui_SelectableBoolPtrEx(const char *label,
                              bool *p_selected,
                              ImGuiSelectableFlags flags /* = 0 */,
                              ImVec2 size /* = ImVec2(0, 0) */); // "bool* p_selected" point to the selection state
    ...
    ImGui_BeginMultiSelect(ImGuiMultiSelectFlags flags); // Implied selection_size = -1, items_count = -1
    CIMGUI_API ImGuiMultiSelectIO *
    ImGui_BeginMultiSelectEx(ImGuiMultiSelectFlags flags, int selection_size /* = -1 */, int items_count /* = -1 */);
    ...
    CIMGUI_API bool ImGui_IsItemToggledSelection(
        void); // Was the last item selection state toggled? Useful if you need the per-item information _before_
    ...
    CIMGUI_API bool ImGui_BeginListBox(const char *label,
    ...
    CIMGUI_API bool ImGui_ListBox(const char *label,
                                  int *current_item,
                                  const char *const items[],
                                  int items_count,
                                  int height_in_items /* = -1 */);
    CIMGUI_API bool ImGui_ListBoxCallback(const char *label,
                                          int *current_item,
                                          const char *(*getter)(void *user_data, int idx),
                                          void *user_data,
                                          int items_count); // Implied height_in_items = -1
    CIMGUI_API bool ImGui_ListBoxCallbackEx(const char *label,
                                            int *current_item,
                                            const char *(*getter)(void *user_data, int idx),
                                            void *user_data,
                                            int items_count,
    ...
    CIMGUI_API void
    ImGui_PlotLines(const char *label,
                    const float *values,
                    int values_count); // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max
                                       // = FLT_MAX, graph_size = ImVec2(0, 0), stride = sizeof(float)
    CIMGUI_API void ImGui_PlotLinesEx(const char *label,
                                      const float *values,
                                      int values_count,
                                      int values_offset /* = 0 */,
    ...
    CIMGUI_API void
    ImGui_PlotLinesCallback(const char *label,
                            float (*values_getter)(void *data, int idx),
                            void *data,
                            int values_count); // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX,
                                               // scale_max = FLT_MAX, graph_size = ImVec2(0, 0)
    CIMGUI_API void ImGui_PlotLinesCallbackEx(const char *label,
                                              float (*values_getter)(void *data, int idx),
                                              void *data,
                                              int values_count,
                                              int values_offset /* = 0 */,
                                              ...
                                              ImVec2 graph_size /* = ImVec2(0, 0) */);
    CIMGUI_API void
    ImGui_PlotHistogram(const char *label,
                        const float *values,
                        int values_count); // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX,
                                           // scale_max = FLT_MAX, graph_size = ImVec2(0, 0), stride = sizeof(float)
    CIMGUI_API void ImGui_PlotHistogramEx(const char *label,
                                          const float *values,
                                          int values_count,
                                          int values_offset /* = 0 */,
                                          const char *overlay_text /* = NULL */,
                                          ...
                                          int stride /* = sizeof(float) */);
    CIMGUI_API void
    ImGui_PlotHistogramCallback(const char *label,
                                float (*values_getter)(void *data, int idx),
                                void *data,
                                int values_count); // Implied values_offset = 0, overlay_text = NULL, scale_min =
                                                   // FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0)
    CIMGUI_API void ImGui_PlotHistogramCallbackEx(const char *label,
                                                  float (*values_getter)(void *data, int idx),
                                                  void *data,
                                                  int values_count,
                                                  int values_offset /* = 0 */,
    ...
    CIMGUI_API bool
    ImGui_BeginMenuEx(const char *label,
    ...
    CIMGUI_API bool ImGui_MenuItemEx(const char *label,
    ...
    CIMGUI_API void ImGui_SetTooltip(const char *fmt, ...)
    ...
    CIMGUI_API bool ImGui_BeginPopupContextItemEx(
        const char *str_id /* = NULL */,
        ImGuiPopupFlags
            popup_flags /* = 1 */); // open+begin popup when clicked on last item. Use str_id==NULL to associate the
    ...
    //  open.
    CIMGUI_API bool ImGui_IsPopupOpen(const char *str_id,
    ...
    // - 5. Call EndTable()
    CIMGUI_API bool
    ImGui_BeginTable(const char *str_id,
                     int columns,
                     ImGuiTableFlags flags /* = 0 */); // Implied outer_size = ImVec2(0.0f, 0.0f), inner_width = 0.0f
    CIMGUI_API bool ImGui_BeginTableEx(const char *str_id,
                                       int columns,
    ...
    CIMGUI_API void ImGui_TableNextRowEx(ImGuiTableRowFlags row_flags /* = 0 */,
                                         float min_row_height /* = 0.0f */); // append into the first cell of a new row.
    CIMGUI_API bool ImGui_TableNextColumn(void); // append into the next column (or first column of next row if
                                                 // currently in last column). Return true when column is visible.
    CIMGUI_API bool
    ImGui_TableSetColumnIndex(int column_n); // append into the specified column. Return true when column is visible.
    ...
    CIMGUI_API void
    ImGui_TableSetupColumn(const char *label,
                           ImGuiTableColumnFlags flags /* = 0 */); // Implied init_width_or_weight = 0.0f, user_id = 0
    CIMGUI_API void ImGui_TableSetupColumnEx(const char *label,
                                             ImGuiTableColumnFlags flags /* = 0 */,
                                             float init_width_or_weight /* = 0.0f */,
                                             ImGuiID user_id /* = 0 */);
    CIMGUI_API void ImGui_TableSetupScrollFreeze(int cols,
    ...
    CIMGUI_API void ImGui_TableAngledHeadersRow(void); // submit a row with angled headers for every column with the
    ...
    ImGui_TableGetSortSpecs(void); // get latest sort specs for the table (NULL if not sorting).  Lifetime: don't hold
                                   // on this pointer over multiple frames or past any subsequent call to BeginTable().
    ...
    ImGui_TableGetColumnName(int column_n /* = -1 */); // return "" if column didn't have a name declared by
    ...
    CIMGUI_API int ImGui_TableGetHoveredColumn(
        void); // return hovered column. return -1 when table is not hovered. return columns_count if the unused space
               // at the right of visible columns is hovered. Can also use (TableGetColumnFlags() &
               // ImGuiTableColumnFlags_IsHovered) instead.
    CIMGUI_API void ImGui_TableSetBgColor(ImGuiTableBgTarget target,
                                          ImU32 color,
    ...
    CIMGUI_API void
    ImGui_NextColumn(void); // next column, defaults to current row or next row if the current row is finished
    CIMGUI_API int ImGui_GetColumnIndex(void); // get current column index
    CIMGUI_API float
    ImGui_GetColumnWidth(int column_index /* = -1 */); // get column width (in pixels). pass -1 to use current column
    CIMGUI_API void ImGui_SetColumnWidth(int column_index,
                                         float width); // set column width (in pixels). pass -1 to use current column
    CIMGUI_API float ImGui_GetColumnOffset(
        int column_index /* = -1 */); // get position of column line (in pixels, from the left side of the contents
                                      // region). pass -1 to use current column, otherwise 0..GetColumnsCount()
                                      // inclusive. column 0 is typically 0.0f
    CIMGUI_API void ImGui_SetColumnOffset(int column_index,
    ...
    // bars/tabs yourself.
    CIMGUI_API bool ImGui_BeginTabBar(const char *str_id,
    ...
    CIMGUI_API bool
    ImGui_BeginTabItem(const char *label,
                       bool *p_open /* = NULL */,
    ...
    CIMGUI_API bool
    ImGui_TabItemButton(const char *label,
    ...
    CIMGUI_API void ImGui_LogToFile(int auto_open_depth /* = -1 */,
    ...
    CIMGUI_API void
    ImGui_EndDragDropSource(void); // only call EndDragDropSource() if BeginDragDropSource() returns true!
    CIMGUI_API bool
    ImGui_BeginDragDropTarget(void); // call after submitting an item that may receive a payload. If this returns true,
                                     // you can call AcceptDragDropPayload() + EndDragDropTarget()
    CIMGUI_API const ImGuiPayload *ImGui_AcceptDragDropPayload(
        const char *type,
        ImGuiDragDropFlags
            flags /* = 0 */); // accept contents of a given type. If ImGuiDragDropFlags_AcceptBeforeDelivery is set you
                              // can peek into the payload before the mouse button is released.
    CIMGUI_API void
    ImGui_EndDragDropTarget(void); // only call EndDragDropTarget() if BeginDragDropTarget() returns true!
    CIMGUI_API const ImGuiPayload *ImGui_GetDragDropPayload(
        void); // peek directly into the current payload from anywhere. returns NULL when drag and drop is finished or
    ...
    CIMGUI_API void
    ImGui_PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect);
    CIMGUI_API void ImGui_PopClipRect(void);

    // Focus, Activation
    CIMGUI_API void
    ImGui_SetItemDefaultFocus(void); // make last item the default focused item of a newly appearing window.
    ...
    CIMGUI_API void
    ImGui_SetNavCursorVisible(bool visible); // alter visibility of keyboard/gamepad cursor. by default: show when using
                                             // an arrow key, hide when clicking with mouse.

    // Overlapping mode
    CIMGUI_API void ImGui_SetNextItemAllowOverlap(
        void); // allow next item to be overlapped by a subsequent item. Useful with invisible buttons, selectable,
               // treenode covering an area where subsequent items may need to be added. Note that both Selectable() and
    ...
    CIMGUI_API bool
    ImGui_IsItemHovered(ImGuiHoveredFlags flags /* = 0 */); // is the last item hovered? (and usable, aka not blocked by
                                                            // a popup, etc.). See ImGuiHoveredFlags for more options.
    CIMGUI_API bool ImGui_IsItemActive(
        void); // is the last item active? (e.g. button being held, text field being edited. This will continuously
    ...
    CIMGUI_API bool ImGui_IsItemClickedEx(
        ImGuiMouseButton mouse_button /* = 0 */); // is the last item hovered and mouse clicked on? (**)  ==
                                                  ...
                                                  // is NOT equivalent to the behavior of e.g. Button(). Read comments
                                                  // in function definition.
    CIMGUI_API bool
    ImGui_IsItemVisible(void); // is the last item visible? (items may be out of sight because of clipping/scrolling)
    CIMGUI_API bool
    ImGui_IsItemEdited(void); // did the last item modify its underlying value this frame? or was pressed? This is
    ...
    CIMGUI_API bool
    ImGui_IsItemDeactivated(void); // was the last item just made inactive (item was previously active). Useful for
                                   // Undo/Redo patterns with widgets that require continuous editing.
    CIMGUI_API bool ImGui_IsItemDeactivatedAfterEdit(
        void); // was the last item just made inactive and made a value change when it was active? (e.g. Slider/Drag
    ...
    CIMGUI_API ImDrawList *ImGui_GetBackgroundDrawList(void); // this draw list will be the first rendered one. Useful
                                                              // to quickly draw shapes/text behind dear imgui contents.
    CIMGUI_API ImDrawList *ImGui_GetForegroundDrawList(void); // this draw list will be the last rendered one. Useful to
                                                              // quickly draw shapes/text over dear imgui contents.

    // Miscellaneous Utilities
    CIMGUI_API bool ImGui_IsRectVisibleBySize(
        ImVec2 size); // test if rectangle (of given size, starting from cursor position) is visible / not clipped.
    CIMGUI_API bool ImGui_IsRectVisible(ImVec2 rect_min,
    ...
    ImGui_GetDrawListSharedData(void); // you may use this when creating your own ImDrawList instances.
    CIMGUI_API const char *
    ImGui_GetStyleColorName(ImGuiCol idx); // get a string corresponding to the enum value (for display, saving, etc.).
    CIMGUI_API void
    ImGui_SetStateStorage(ImGuiStorage *storage); // replace current window storage with our own (if you want to
    ...
    CIMGUI_API ImVec2 ImGui_CalcTextSize(
        const char *text); // Implied text_end = NULL, hide_text_after_double_hash = false, wrap_width = -1.0f
    CIMGUI_API ImVec2 ImGui_CalcTextSizeEx(const char *text,
    ...
    CIMGUI_API bool
    ImGui_IsKeyPressedEx(ImGuiKey key, bool repeat /* = true */); // was key pressed (went from !Down to Down)? if
    ...
    CIMGUI_API bool ImGui_IsKeyChordPressed(
        ImGuiKeyChord key_chord); // was key chord (mods + key) pressed, e.g. you can pass 'ImGuiMod_Ctrl | ImGuiKey_S'
                                  // as a key-chord. This doesn't do any routing or focus check, please consider using
                                  // Shortcut() function instead.
    CIMGUI_API int
    ImGui_GetKeyPressedAmount(ImGuiKey key,
                              float repeat_delay,
    ...
    CIMGUI_API const char *
    ImGui_GetKeyName(ImGuiKey key); // [DEBUG] returns English name of the key. Those names are provided for debugging
    ...
    CIMGUI_API void
    ImGui_SetItemKeyOwner(ImGuiKey key); // Set key owner to last item ID if it is hovered or active. Equivalent to 'if
    ...
    CIMGUI_API bool ImGui_IsMouseClickedEx(ImGuiMouseButton button,
                                           bool repeat /* = false */); // did mouse button clicked? (went from !Down to
                                                                       // Down). Same as GetMouseClickedCount() == 1.
    CIMGUI_API bool
    ImGui_IsMouseReleased(ImGuiMouseButton button); // did mouse button released? (went from Down to !Down)
    ...
    CIMGUI_API bool ImGui_IsMouseReleasedWithDelay(
        ...
        float
            delay); // delayed mouse release (use very sparingly!). Generally used with 'delay >=
    ...
    CIMGUI_API int
    ImGui_GetMouseClickedCount(ImGuiMouseButton button); // return the number of successive mouse-clicks at the time
                                                         // where a click happen (otherwise 0).
    CIMGUI_API bool ImGui_IsMouseHoveringRect(ImVec2 r_min, ImVec2 r_max); // Implied clip = true
    CIMGUI_API bool
    ImGui_IsMouseHoveringRectEx(ImVec2 r_min,
                                ImVec2 r_max,
    ...
    CIMGUI_API bool
    ImGui_IsMousePosValid(const ImVec2 *mouse_pos /* = NULL */); // by convention we use (-FLT_MAX,-FLT_MAX) to denote
                                                                 // that there is no mouse available
    CIMGUI_API bool ImGui_IsAnyMouseDown(
        void); // [WILL OBSOLETE] is any mouse button held? This was designed for backends, but prefer having backend
               // maintain a mask of held mouse buttons, because upcoming input queue system will make this invalid.
    CIMGUI_API ImVec2
    ImGui_GetMousePos(void); // shortcut to ImGui::GetIO().MousePos provided by user, to be consistent with other calls
    CIMGUI_API ImVec2 ImGui_GetMousePosOnOpeningCurrentPopup(
        void); // retrieve mouse position at the time of opening popup we have BeginPopup() into (helper to avoid user
               // backing that value themselves)
    CIMGUI_API bool
    ImGui_IsMouseDragging(ImGuiMouseButton button,
                          float lock_threshold /* = -1.0f */); // is mouse dragging? (uses io.MouseDraggingThreshold if
                                                               // lock_threshold < 0.0f)
    CIMGUI_API ImVec2 ImGui_GetMouseDragDelta(
        ...
        float lock_threshold /* = -1.0f */); // return the delta from the initial clicking position while the mouse
                                             ...
                                             // until the mouse moves past a distance threshold at least once (uses
    ...
    ImGui_GetMouseCursor(void); // get desired mouse cursor shape. Important: reset in ImGui::NewFrame(), this is
                                // updated during the frame. valid before Render(). If you use software rendering by
    ...
    CIMGUI_API void ImGui_SetNextFrameWantCaptureMouse(
        bool want_capture_mouse); // Override io.WantCaptureMouse flag next frame (said flag is left for your
                                  // application to handle, typical when true it instructs your app to ignore inputs).
    ...
    CIMGUI_API bool
    ImGui_DebugCheckVersionAndDataLayout(const char *version_str,
                                         size_t sz_io,
                                         size_t sz_style,
                                         size_t sz_vec2,
                                         size_t sz_vec4,
    ...
    //   for more details.
    CIMGUI_API void
    ImGui_SetAllocatorFunctions(ImGuiMemAllocFunc alloc_func, ImGuiMemFreeFunc free_func, void *user_data /* = NULL */);
    CIMGUI_API void
    ImGui_GetAllocatorFunctions(ImGuiMemAllocFunc *p_alloc_func, ImGuiMemFreeFunc *p_free_func, void **p_user_data);
    ...
    typedef enum
    {
        ...
        ImGuiItemFlags_ButtonRepeat =
            1 << 3, // false    // Any button-like behavior will have repeat mode enabled (based on io.KeyRepeatDelay
                    // and io.KeyRepeatRate values). Note that you can also call IsItemActive() after any button to tell
                    // if it is being held.
    ...
    typedef enum
    {
        ...
        ImGuiTreeNodeFlags_DrawLinesFull = 1
                                           << 19, // Horizontal lines to child nodes. Vertical line drawn down to
    ...
    typedef enum
    {
        ...
        ImGuiPopupFlags_NoOpenOverExistingPopup = 1
                                                  << 7, // For OpenPopup*(), BeginPopupContext*(): don't open if there's
    ...
    typedef enum
    {
        ...
        // ImGuiFocusedFlags_DockHierarchy               = 1 << 4,   // Consider docking hierarchy (treat dockspace host
    ...
    typedef enum
    {
        ...
        // hierarchy (treat dockspace host as parent of docked window) (when used with _ChildWindows or _RootWindow)
...
#define IMGUI_PAYLOAD_TYPE_COLOR_4F "_COL4F" // float[4]: Standard type for colors. User code may use this type.
    ...
    // concept as input "Characters", the later are submitted via io.AddInputCharacter(). The keyboard key enum values
    ...
    typedef enum
    {
        ...
        ImGuiColorEditFlags_NoSmallPreview =
            1 << 4, //              // ColorEdit, ColorPicker: disable color square preview next to the inputs. (e.g. to
        ...
        ImGuiColorEditFlags_AlphaNoBg = 1
                                        << 12, //              // ColorEdit, ColorPicker, ColorButton: disable rendering
                                               //              a checkerboard background behind transparent color.
        ImGuiColorEditFlags_AlphaPreviewHalf = 1 << 13, //              // ColorEdit, ColorPicker, ColorButton: display
    ...
    CIMGUI_API void
    ImVector_Construct(void *vector); // Construct a zero-size ImVector<> (of any type). This is primarily useful when
                                      // calling ImFontGlyphRangesBuilder_BuildRanges()
    CIMGUI_API void
    ImVector_Destruct(void *vector); // Destruct an ImVector<> (of any type). Important: Frees the vector memory but
                                     // does not call destructors on contained objects (if they have them)

#if defined(IMGUI_HAS_IMSTR)
#if IMGUI_HAS_IMSTR
    CIMGUI_API ImStr
    ImStr_FromCharStr(const char *b); // Build an ImStr from a regular const char* (no data is copied, so you need to
                                      // make sure the original char* isn't altered as long as you are using the ImStr).
...
#define CIM_FREE(_PTR) ImGui_MemFree(_PTR)
    ...
    IM_MSVC_RUNTIME_CHECKS_OFF
    struct ImVector_ImGuiTextRange_t
    {
        int Size;
        int Capacity;
    ...
    struct ImVector_ImTextureRef_t
    {
        int Size;
        int Capacity;
        ImTextureRef *Data;
    ...
    struct ImVector_ImTextureRect_t
    {
        int Size;
        int Capacity;
        ImTextureRect *Data;
    ...
    struct ImVector_ImDrawListSharedDataPtr_t
    {
        int Size;
        int Capacity;
    ...
    struct ImVector_ImFontConfigPtr_t
    {
        int Size;
        int Capacity;
        ImFontConfig **Data;
    }; // Instantiation of ImVector<ImFontConfig*>
    struct ImVector_ImTextureDataPtr_t
    {
        int Size;
        int Capacity;
        ImTextureData **Data;
    ...
    struct ImGuiStyle_t
    {
        ...
        float FontSizeBase;  // Current base font size before external global factors are applied. Use PushFont(NULL,
        ...
        float WindowBorderHoverPadding; // Hit-testing extent outside/inside resizing border. Also extend determination
                                        // of hovered window. Generally meaningfully larger than WindowBorderSize to
                                        // make it easy to reach borders.
        ...
        bool AntiAliasedLines; // Enable anti-aliased lines/borders. Disable if you are really tight on CPU/GPU. Latched
                               // at the beginning of the frame (copied to ImDrawList).
        bool AntiAliasedLinesUseTex; // Enable anti-aliased lines/borders using textures where possible. Require backend
                                     // to render with bilinear filtering (NOT point/nearest filtering). Latched at the
    ...
    CIMGUI_API void
    ImGuiStyle_ScaleAllSizes(ImGuiStyle *self,
    ...
    struct ImGuiIO_t
    {
        ...
        bool ConfigDragClickToInputText; // = false          // [BETA] Enable turning DragXXX widgets into text input
                                         // with a simple mouse click-release (without moving). Not desirable on devices
                                         // without a keyboard.
        bool ConfigWindowsResizeFromEdges; // = true           // Enable resizing of windows from their edges and from
                                           // the lower-left corner. This requires ImGuiBackendFlags_HasMouseCursors for
                                           // better mouse cursor feedback. (This used to be a per-window
        ...
        bool ConfigWindowsCopyContentsWithCtrlC; // = false      // [EXPERIMENTAL] CTRL+C copy the contents of focused
                                                 // window into the clipboard. Experimental because: (1) has known
                                                 // issues with nested Begin/End pairs (2) text output quality varies
        ...
        bool ConfigErrorRecovery; // = true       // Enable error recovery support. Some errors won't be detected and
                                  // lead to direct crashes if recovery is disabled.
        ...
        bool ConfigErrorRecoveryEnableTooltip;  // = true       // Enable tooltip on recoverable errors. The tooltip
                                                // include a way to enable asserts if they were disabled.
        ...
        bool
            WantCaptureKeyboard; // Set when Dear ImGui will use keyboard inputs, in this case do not dispatch them to
        ...
        bool
            WantTextInput; // Mobile/console: when set, you may display an on-screen keyboard. This is set by Dear ImGui
        ...
        bool WantSetMousePos; // MousePos has been altered, backend should reposition mouse on next frame. Rarely used!
                              // Set only when io.ConfigNavMoveSetMousePos is enabled.
        ...
        ImVec2 MousePosPrev; // Previous mouse position (note that MouseDelta is not necessary == MousePos-MousePosPrev,
                             // in case either position is invalid)
        ...
        double MouseReleasedTime[5];    // Time of last released (rarely used! but useful to handle delayed single-click
                                        // when trying to disambiguate them from double-click).
        ...
        ImVector_ImWchar InputQueueCharacters; // Queue of _characters_ input (obtained by platform backend). Fill using
        ...
        // entries array which represent your "native" keyboard state. The first 512 are now unused and should be kept
        // zero. Legacy backend will write into KeyMap[] using ImGuiKey_ indices which are always >512. bool
        // KeysDown[ImGuiKey_COUNT];           // [LEGACY] Input: Keyboard keys that are pressed (ideally left in the
        // "native" order your engine has access to keyboard keys, so you can use your own defines/enums for keys). This
        // used to be [512] sized. It is now ImGuiKey_COUNT to allow legacy io.KeysDown[GetKeyIndex(...)] to work
        // without an overflow. float     NavInputs[ImGuiNavInput_COUNT];     // [LEGACY] Since 1.88, NavInputs[] was
        // removed. Backends from 1.60 to 1.86 won't build. Feed gamepad inputs via io.AddKeyEvent() and
        // ImGuiKey_GamepadXXX enums. void*     ImeWindowHandle;                    // [Obsoleted in 1.87] Set
        // ImGuiViewport::PlatformHandleRaw instead. Set this to your HWND to get automatic IME cursor positioning.
    ...
    // Input Functions
    CIMGUI_API void
    ImGuiIO_AddKeyEvent(ImGuiIO *self,
                        ImGuiKey key,
    ...
    CIMGUI_API void
    ImGuiIO_AddKeyAnalogEvent(ImGuiIO *self,
                              ImGuiKey key,
                              bool down,
    ...
    CIMGUI_API void ImGuiIO_AddMousePosEvent(ImGuiIO *self,
                                             float x,
    ...
    CIMGUI_API void ImGuiIO_AddMouseWheelEvent(
        ImGuiIO *self, float wheel_x, float wheel_y); // Queue a mouse wheel update. wheel_y<0: scroll down, wheel_y>0:
                                                      // scroll up, wheel_x<0: scroll right, wheel_x>0: scroll left.
    CIMGUI_API void
    ImGuiIO_AddMouseSourceEvent(ImGuiIO *self,
                                ImGuiMouseSource source); // Queue a mouse source change (Mouse/TouchScreen/Pen)
    CIMGUI_API void ImGuiIO_AddFocusEvent(ImGuiIO *self,
    ...
    CIMGUI_API void ImGuiIO_AddInputCharacterUTF16(
        ImGuiIO *self, ImWchar16 c); // Queue a new character input from a UTF-16 character, it can be a surrogate
    CIMGUI_API void ImGuiIO_AddInputCharactersUTF8(ImGuiIO *self,
    ...
    CIMGUI_API void ImGuiIO_ClearInputKeys(ImGuiIO *self);  // Clear current keyboard/gamepad state + current frame text
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    CIMGUI_API void ImGuiIO_ClearInputCharacters(ImGuiIO *self); // [Obsoleted in 1.89.8] Clear the current frame text
...
#define IM_UNICODE_CODEPOINT_MAX 0xFFFF // Maximum Unicode code point supported by this build.
    ...
    CIMGUI_API void ImGuiTextFilter_ImGuiTextRange_split(const ImGuiTextFilter_ImGuiTextRange *self,
                                                         char separator,
    ...
    CIMGUI_API bool
    ImGuiTextFilter_PassFilter(const ImGuiTextFilter *self, const char *text, const char *text_end /* = NULL */);
    ...
    CIMGUI_API const char *ImGuiTextBuffer_end(
        const ImGuiTextBuffer *self); // Buf is zero-terminated, so end() will point on the zero-terminator
    ...
    CIMGUI_API void
    ImGuiTextBuffer_resize(ImGuiTextBuffer *self,
                           int size); // Similar to resize(0) on ImVector: empty string but don't free buffer.
    ...
    CIMGUI_API void
    ImGuiListClipper_End(ImGuiListClipper *self); // Automatically called on the last call of Step() that returns false.
    CIMGUI_API bool
    ImGuiListClipper_Step(ImGuiListClipper *self); // Call until it returns false. The DisplayStart/DisplayEnd fields
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    CIMGUI_API void
    ImGuiListClipper_IncludeRangeByIndices(ImGuiListClipper *self, int item_begin, int item_end); // [renamed in 1.89.9]
#endif // #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
#define IM_COL32_BLACK_TRANS IM_COL32(0, 0, 0, 0)   // Transparent black = 0x00000000
    ...
    CIMGUI_API bool ImGuiSelectionBasicStorage_Contains(const ImGuiSelectionBasicStorage *self,
    ...
    CIMGUI_API ImGuiID ImGuiSelectionBasicStorage_GetStorageIdFromIndex(
        ImGuiSelectionBasicStorage *self, int idx); // Convert index to item id based on provided adapter.
...
#define ImDrawCallback_ResetRenderState (ImDrawCallback)(-8)
    ...
    struct ImDrawCmd_t
    {
        ...
        ImTextureRef TexRef; // 16   // Reference to a font/texture atlas (where backend called
                             // ImTextureData::SetTexID()) or to a user-provided texture ID (via e.g. ImGui::Image()
    ...
    ImDrawCmd_GetTexID(const ImDrawCmd *self); // == (TexRef._TexData ? TexRef._TexData->TexID : TexRef._TexID
...
// Vertex layout
    ...
    // can either declare the struct or use a typedef). This is because ImVec2/ImU32 are likely not declared at the time
    ...
    CIMGUI_API void ImDrawListSplitter_Clear(
        ImDrawListSplitter *self); // Do not clear Channels[] so our allocations are reused next frame
    ...
    CIMGUI_API void
    ImDrawListSplitter_SetCurrentChannel(ImDrawListSplitter *self, ImDrawList *draw_list, int channel_idx);
    ...
    struct ImDrawList_t
    {
        ...
        // thickness, int num_segments = 0) { AddBezierCubic(p1, p2, p3, p4, col, thickness, num_segments); } //
    ...
    CIMGUI_API void
    ImDrawList_AddLineEx(ImDrawList *self, ImVec2 p1, ImVec2 p2, ImU32 col, float thickness /* = 1.0f */);
    CIMGUI_API void ImDrawList_AddRect(ImDrawList *self,
                                       ImVec2 p_min,
                                       ImVec2 p_max,
                                       ImU32 col); // Implied rounding = 0.0f, flags = 0, thickness = 1.0f
    CIMGUI_API void
    ImDrawList_AddRectEx(ImDrawList *self,
                         ImVec2 p_min,
                         ImVec2 p_max,
                         ImU32 col,
                         float rounding /* = 0.0f */,
    ...
    CIMGUI_API void
    ImDrawList_AddRectFilledEx(ImDrawList *self,
                               ImVec2 p_min,
                               ImVec2 p_max,
                               ImU32 col,
                               float rounding /* = 0.0f */,
    ...
    CIMGUI_API void ImDrawList_AddQuad(
        ImDrawList *self, ImVec2 p1, ImVec2 p2, ImVec2 p3, ImVec2 p4, ImU32 col); // Implied thickness = 1.0f
    CIMGUI_API void ImDrawList_AddQuadEx(
        ImDrawList *self, ImVec2 p1, ImVec2 p2, ImVec2 p3, ImVec2 p4, ImU32 col, float thickness /* = 1.0f */);
    CIMGUI_API void ImDrawList_AddQuadFilled(ImDrawList *self, ImVec2 p1, ImVec2 p2, ImVec2 p3, ImVec2 p4, ImU32 col);
    CIMGUI_API void
    ImDrawList_AddTriangle(ImDrawList *self, ImVec2 p1, ImVec2 p2, ImVec2 p3, ImU32 col); // Implied thickness = 1.0f
    CIMGUI_API void ImDrawList_AddTriangleEx(
        ImDrawList *self, ImVec2 p1, ImVec2 p2, ImVec2 p3, ImU32 col, float thickness /* = 1.0f */);
    ...
    CIMGUI_API void ImDrawList_AddCircleEx(ImDrawList *self,
                                           ImVec2 center,
                                           float radius,
                                           ImU32 col,
                                           int num_segments /* = 0 */,
    ...
    CIMGUI_API void
    ImDrawList_AddCircleFilled(ImDrawList *self, ImVec2 center, float radius, ImU32 col, int num_segments /* = 0 */);
    CIMGUI_API void ImDrawList_AddNgon(
        ImDrawList *self, ImVec2 center, float radius, ImU32 col, int num_segments); // Implied thickness = 1.0f
    CIMGUI_API void ImDrawList_AddNgonEx(
        ImDrawList *self, ImVec2 center, float radius, ImU32 col, int num_segments, float thickness /* = 1.0f */);
    CIMGUI_API void
    ImDrawList_AddNgonFilled(ImDrawList *self, ImVec2 center, float radius, ImU32 col, int num_segments);
    CIMGUI_API void ImDrawList_AddEllipse(ImDrawList *self,
                                          ImVec2 center,
                                          ImVec2 radius,
                                          ImU32 col); // Implied rot = 0.0f, num_segments = 0, thickness = 1.0f
    CIMGUI_API void ImDrawList_AddEllipseEx(ImDrawList *self,
                                            ImVec2 center,
                                            ImVec2 radius,
                                            ImU32 col,
                                            float rot /* = 0.0f */,
    ...
    CIMGUI_API void ImDrawList_AddEllipseFilledEx(
        ImDrawList *self, ImVec2 center, ImVec2 radius, ImU32 col, float rot /* = 0.0f */, int num_segments /* = 0 */);
    CIMGUI_API void
    ImDrawList_AddText(ImDrawList *self, ImVec2 pos, ImU32 col, const char *text_begin); // Implied text_end = NULL
    CIMGUI_API void ImDrawList_AddTextEx(
        ImDrawList *self, ImVec2 pos, ImU32 col, const char *text_begin, const char *text_end /* = NULL */);
    CIMGUI_API void ImDrawList_AddTextImFontPtr(
        ...
        ImVec2 pos,
        ImU32 col,
        const char *text_begin); // Implied text_end = NULL, wrap_width = 0.0f, cpu_fine_clip_rect = NULL
    CIMGUI_API void ImDrawList_AddTextImFontPtrEx(ImDrawList *self,
                                                  ImFont *font,
                                                  float font_size,
                                                  ImVec2 pos,
                                                  ImU32 col,
    ...
    CIMGUI_API void
    ImDrawList_AddImage(ImDrawList *self,
                        ImTextureRef tex_ref,
                        ImVec2 p_min,
                        ImVec2 p_max); // Implied uv_min = ImVec2(0, 0), uv_max = ImVec2(1, 1), col = IM_COL32_WHITE
    ...
    CIMGUI_API void ImDrawList_AddImageQuadEx(ImDrawList *self,
                                              ImTextureRef tex_ref,
                                              ImVec2 p1,
                                              ImVec2 p2,
                                              ImVec2 p3,
    ...
    CIMGUI_API void
    ImDrawList_PathStroke(ImDrawList *self, ImU32 col, ImDrawFlags flags /* = 0 */, float thickness /* = 1.0f */);
    CIMGUI_API void ImDrawList_PathArcTo(
        ImDrawList *self, ImVec2 center, float radius, float a_min, float a_max, int num_segments /* = 0 */);
    ...
    CIMGUI_API void ImDrawList_PathEllipticalArcToEx(ImDrawList *self,
                                                     ImVec2 center,
                                                     ImVec2 radius,
                                                     float rot,
                                                     float a_min,
    ...
    CIMGUI_API void ImDrawList_PathBezierQuadraticCurveTo(
        ImDrawList *self, ImVec2 p2, ImVec2 p3, int num_segments /* = 0 */); // Quadratic Bezier (3 control points)
    CIMGUI_API void ImDrawList_PathRect(
        ImDrawList *self, ImVec2 rect_min, ImVec2 rect_max, float rounding /* = 0.0f */, ImDrawFlags flags /* = 0 */);
    ...
    CIMGUI_API void
    ImDrawList_AddCallback(ImDrawList *self, ImDrawCallback callback, void *userdata); // Implied userdata_size = 0
    CIMGUI_API void
    ImDrawList_AddCallbackEx(ImDrawList *self, ImDrawCallback callback, void *userdata, size_t userdata_size /* = 0 */);
    // Advanced: Miscellaneous
    CIMGUI_API void ImDrawList_AddDrawCmd(
        ImDrawList
            *self); // This is useful if you need to forcefully create a new draw call (to allow for dependent rendering
                    // / blending). Otherwise primitives are merged into the same draw-call as much as possible
    CIMGUI_API ImDrawList *
    ImDrawList_CloneOutput(const ImDrawList *self); // Create a clone of the CmdBuffer/IdxBuffer/VtxBuffer.
    // Advanced: Channels
    ...
    CIMGUI_API void
    ImDrawList_PrimVtx(ImDrawList *self, ImVec2 pos, ImVec2 uv, ImU32 col); // Write vertex with unique index
// Obsolete names
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ...
    CIMGUI_API void ImDrawList__PathArcToFastEx(
        ImDrawList *self, ImVec2 center, float radius, int a_min_sample, int a_max_sample, int a_step);
    CIMGUI_API void
    ImDrawList__PathArcToN(ImDrawList *self, ImVec2 center, float radius, float a_min, float a_max, int num_segments);
    ...
    struct ImDrawData_t
    {
        ...
        ImVector_ImDrawListPtr CmdLists; // Array of ImDrawList* to render. The ImDrawLists are owned by ImGuiContext
        ...
        ImGuiViewport *OwnerViewport; // Viewport carrying the ImDrawData instance, might be of use to the renderer
                                      // (generally not).
        ImVector_ImTextureDataPtr *Textures; // List of textures to update. Most of the times the list is shared by all
                                             // ImDrawData, has only 1 texture and it doesn't need any update. This
                                             // almost always points to ImGui::GetPlatformIO().Textures[]. May be
                                             // overriden or set to NULL if you want to manually update textures.
    };
    CIMGUI_API void ImDrawData_Clear(ImDrawData *self);
    CIMGUI_API void
    ImDrawData_AddDrawList(ImDrawData *self,
    ...
    CIMGUI_API void ImDrawData_DeIndexAllBuffers(
        ImDrawData
            *self); // Helper to convert all buffers from indexed to non-indexed, in case you cannot render indexed.
    ...
    CIMGUI_API void ImDrawData_ScaleClipRects(
        ImDrawData *self,
        ImVec2 fb_scale); // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output buffer is at
                          // a different scale than Dear ImGui expects, or if there is a difference between your window
...
#undef Status // X11 headers are leaking this. // We intentionally support a limited amount of texture formats to limit
              // burden on CPU-side code and extension.
    ...
    CIMGUI_API void ImTextureData_SetTexID(
        ImTextureData *self,
        ImTextureID tex_id); // Call after creating or destroying the texture. Never modify TexID directly!
    CIMGUI_API void
    ImTextureData_SetStatus(ImTextureData *self,
                            ImTextureStatus status); // Call after honoring a request. Never modify Status directly!
    ...
    struct ImFontConfig_t
    {
        ...
        ImS8 OversampleV; // 0 (1)    // Rasterize at higher quality for sub-pixel positioning. 0 == auto == 1. This is
        ...
        const ImWchar *GlyphRanges; // NULL     // *LEGACY* THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT IS
                                    // ALIVE. Pointer to a user-provided list of Unicode range (2 value per range,
                                    // values are inclusive, zero-terminated list).
        const ImWchar *GlyphExcludeRanges; // NULL     // Pointer to a small user-provided list of Unicode ranges (2
                                           // value per range, values are inclusive, zero-terminated list). This is very
                                           // close to GlyphRanges[] but designed to exclude ranges from a font source,
                                           // when merging fonts with overlapping glyphs. Use "Input Glyphs Overlap
                                           // Detection Tool" to find about your overlapping ranges.
        // ImVec2        GlyphExtraSpacing;      // 0, 0     // (REMOVED AT IT SEEMS LARGELY OBSOLETE. PLEASE REPORT IF
        // YOU WERE USING THIS). Extra spacing (in pixels) between glyphs when rendered: essentially add to
        ...
        float GlyphMinAdvanceX; // 0        // Minimum AdvanceX for glyphs, set Min to align font icons, set both
                                // Min/Max to enforce mono-space font. Absolute value for default size, other sizes will
                                // scale this value.
        ...
        float RasterizerDensity;  // 1.0f     // [LEGACY: this only makes sense when
                                  // ImGuiBackendFlags_RendererHasTextures is not supported] DPI scale multiplier for
        // rasterization. Not altering other font metrics: makes it easy to swap between e.g. a
        // 100% and a 400% fonts for a zooming display, or handle Retina screen. IMPORTANT: If
        // you change this it is expected that you increase/decrease font scale roughly to the
    ...
    CIMGUI_API bool ImFontGlyphRangesBuilder_GetBit(const ImFontGlyphRangesBuilder *self,
    ...
    CIMGUI_API void ImFontGlyphRangesBuilder_BuildRanges(
        ImFontGlyphRangesBuilder *self,
        ImVector_ImWchar *out_ranges); // Output new ranges (ImVector_Construct()/ImVector_Destruct() can be used to
                                       // safely construct out_ranges)
...
#define ImFontAtlasRectId_Invalid -1
    ...
    struct ImFontAtlas_t
    {
        ...
        int TexGlyphPadding; // FIXME: Should be called "TexPackPadding". Padding between glyphs within texture in
                             // pixels. Defaults to 1. If your rendering method doesn't rely on bilinear filtering you
                             // may set this to 0 (will also need to set AntiAliasedLinesUseTex = false).
        ...
        ImVector_ImFontPtr Fonts; // Hold all the fonts returned by AddFont*. Fonts[0] is the default font upon calling
        ...
        // FontLoaderFlags. int                               TexDesiredWidth;         // OBSOLETED in 1.92.X: Force
        // texture width before calling Build(). Must be a power-of-two. If have many glyphs your graphics API have
        // texture size restrictions you may want to increase texture width to decrease height) typedef ImFontAtlasRect
        // ImFontAtlasCustomRect;   // OBSOLETED in 1.92.X typedef ImFontAtlasCustomRect     CustomRect;              //
    ...
    CIMGUI_API ImFont *ImFontAtlas_AddFontFromMemoryTTF(
        ...
        const ImFontConfig *font_cfg /* = NULL */,
        const ImWchar
            *glyph_ranges /* = NULL */); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after
    ...
    CIMGUI_API void ImFontAtlas_SetFontLoader(ImFontAtlas *self,
                                              const ImFontLoader *font_loader); // Change font loader at runtime.
    // As we are transitioning toward a new font system, we expect to obsolete those soon:
    CIMGUI_API void ImFontAtlas_ClearInputData(
        ImFontAtlas *self); // [OBSOLETE] Clear input data (all ImFontConfig structures including sizes, TTF data, glyph
                            // ranges, etc.) = all the data used to build the texture and fonts.
    CIMGUI_API void ImFontAtlas_ClearFonts(ImFontAtlas *self); // [OBSOLETE] Clear input+output font data (same as
                                                               // ClearInputData() + glyphs storage, UV coordinates).
    CIMGUI_API void
    ImFontAtlas_ClearTexData(ImFontAtlas *self); // [OBSOLETE] Clear CPU-side copy of the texture data. Saves RAM once
    ...
    CIMGUI_API bool ImFontAtlas_Build(
        ImFontAtlas *self); // Build pixels data. This is called automatically for you by the GetTexData*** functions.
    ...
    CIMGUI_API void
    ImFontAtlas_SetTexID(ImFontAtlas *self,
                         ImTextureID id); // Called by legacy backends. May be called before texture creation.
    ...
    CIMGUI_API bool ImFontAtlas_IsBuilt(
        const ImFontAtlas *self); // Bit ambiguous: used to detect when user didn't build texture but effectively we
    ...
    CIMGUI_API const ImWchar *ImFontAtlas_GetGlyphRangesJapanese(
        ImFontAtlas *self); // Default + Hiragana, Katakana, Half-Width, Selection of 2999 Ideographs
    CIMGUI_API const ImWchar *
    ImFontAtlas_GetGlyphRangesChineseFull(ImFontAtlas *self); // Default + Half-Width + Japanese Hiragana/Katakana +
                                                              // full set of about 21000 CJK Unified Ideographs
    CIMGUI_API const ImWchar *ImFontAtlas_GetGlyphRangesChineseSimplifiedCommon(
        ImFontAtlas *self); // Default + Half-Width + Japanese Hiragana/Katakana + set of 2500 CJK Unified Ideographs
                            // for common simplified Chinese
    CIMGUI_API const ImWchar *
    ImFontAtlas_GetGlyphRangesCyrillic(ImFontAtlas *self); // Default + about 400 Cyrillic characters
    CIMGUI_API const ImWchar *ImFontAtlas_GetGlyphRangesThai(ImFontAtlas *self); // Default + Thai characters
    CIMGUI_API const ImWchar *
    ImFontAtlas_GetGlyphRangesVietnamese(ImFontAtlas *self); // Default + Vietnamese characters
#endif                                                       // #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ...
    CIMGUI_API void
    ImFontAtlas_RemoveCustomRect(ImFontAtlas *self,
                                 ImFontAtlasRectId id); // Unregister a rectangle. Existing pixels will stay in texture
                                                        // until resized / garbage collected.
    CIMGUI_API bool ImFontAtlas_GetCustomRect(
        const ImFontAtlas *self,
        ImFontAtlasRectId id,
        ImFontAtlasRect
            *out_r); // Get rectangle coordinates for current texture. Valid immediately, never store this (read above)!
// [Obsolete]
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    CIMGUI_API ImFontAtlasRectId ImFontAtlas_AddCustomRectRegular(ImFontAtlas *self, int w, int h); // RENAMED in 1.92.X
    CIMGUI_API const ImFontAtlasRect *ImFontAtlas_GetCustomRectByIndex(ImFontAtlas *self,
                                                                       ImFontAtlasRectId id); // OBSOLETED in 1.92.X
    CIMGUI_API void ImFontAtlas_CalcCustomRectUV(const ImFontAtlas *self,
                                                 const ImFontAtlasRect *r,
                                                 ImVec2 *out_uv_min,
                                                 ImVec2 *out_uv_max); // OBSOLETED in 1.92.X
    CIMGUI_API ImFontAtlasRectId ImFontAtlas_AddCustomRectFontGlyph(
        ImFontAtlas *self,
        ImFont *font,
        ImWchar codepoint,
        int w,
        int h,
        float advance_x,
        ImVec2 offset /* = ImVec2(0, 0) */); // OBSOLETED in 1.92.X: Use custom ImFontLoader in ImFontConfig
    CIMGUI_API ImFontAtlasRectId
    ImFontAtlas_AddCustomRectFontGlyphForSize(ImFontAtlas *self,
                                              ImFont *font,
                                              float font_size,
                                              ImWchar codepoint,
                                              int w,
                                              int h,
                                              float advance_x,
                                              ImVec2 offset /* = ImVec2(0, 0) */); // ADDED AND OBSOLETED in 1.92.X
#endif // #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ...
    struct ImFontBaked_t
    {
        ...
        unsigned int
            LoadNoRenderOnLayout : 1; // 0  //     // Enable a two-steps mode where CalcTextSize() calls will load
                                      // AdvanceX *without* rendering/packing glyphs. Only advantagous if you know that
                                      // the glyph is unlikely to actually be rendered, otherwise it is slower because
                                      // we'd do one query on the first CalcTextSize and one query on the first Draw.
    ...
    CIMGUI_API ImFontGlyph *ImFontBaked_FindGlyph(ImFontBaked *self,
                                                  ImWchar c); // Return U+FFFD glyph if requested glyph doesn't exists.
    CIMGUI_API ImFontGlyph *ImFontBaked_FindGlyphNoFallback(ImFontBaked *self,
    ...
    struct ImFont_t
    {
        ...
        float
            LegacySize; // 4     // in  // Font size passed to AddFont(). Use for old code calling PushFont() expecting
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
        float Scale; // 4     // in  // Legacy base font scale (~1.0f), multiplied by the per-window font scale which
    ...
    CIMGUI_API ImFontBaked *ImFont_GetFontBakedEx(
        ImFont *self, float font_size, float density /* = -1.0f */); // Get or create baked data for given size
    CIMGUI_API ImVec2 ImFont_CalcTextSizeA(ImFont *self,
                                           float size,
                                           float max_width,
                                           float wrap_width,
                                           const char *text_begin); // Implied text_end = NULL, remaining = NULL
    CIMGUI_API ImVec2 ImFont_CalcTextSizeAEx(ImFont *self,
                                             float size,
                                             float max_width,
                                             float wrap_width,
                                             const char *text_begin,
    ...
    CIMGUI_API const char *
    ImFont_CalcWordWrapPosition(ImFont *self, float size, const char *text, const char *text_end, float wrap_width);
    CIMGUI_API void ImFont_RenderChar(ImFont *self,
                                      ImDrawList *draw_list,
                                      float size,
                                      ImVec2 pos,
                                      ImU32 col,
                                      ImWchar c); // Implied cpu_fine_clip = NULL
    CIMGUI_API void ImFont_RenderCharEx(ImFont *self,
                                        ImDrawList *draw_list,
                                        float size,
                                        ImVec2 pos,
                                        ImU32 col,
    ...
    CIMGUI_API void
    ImFont_AddRemapChar(ImFont *self,
    ...
    struct ImGuiPlatformIO_t
    {
        ...
        // shutdown.
        ImVector_ImTextureDataPtr Textures; // List of textures used by Dear ImGui (most often 1) + contents of external
    ...
    struct ImGuiPlatformImeData_t
    {
        ...
        bool WantTextInput; // A widget wants text input, not necessarily IME to be visible. This is automatically set
                            // to the upcoming value of io.WantTextInput.
    ...
    CIMGUI_API void
    ImGui_ImageImVec4(ImTextureRef tex_ref,
                      ImVec2 image_size,
                      ImVec2 uv0,
                      ImVec2 uv1,
                      ImVec4 tint_col,
                      ImVec4 border_col); // <-- 'border_col' was removed in favor of ImGuiCol_ImageBorder. If you use
    ...
    CIMGUI_API ImVec2 ImGui_GetContentRegionMax(
        void); // Content boundaries max (e.g. window boundaries including scrolling, or current column boundaries). You
    ...
    CIMGUI_API ImVec2 ImGui_GetWindowContentRegionMin(
        void); // Content boundaries min for the window (roughly (0,0)-Scroll), in window-local coordinates. You should
               // never need this. Always use GetCursorScreenPos() and GetContentRegionAvail()!
    CIMGUI_API ImVec2 ImGui_GetWindowContentRegionMax(
        void); // Content boundaries max for the window (roughly (0,0)+Size-Scroll), in window-local coordinates. You
               // should never need this. Always use GetCursorScreenPos() and GetContentRegionAvail()!
    ...
    // const ImVec2& size_arg, bool borders, ImGuiWindowFlags window_flags)        { return BeginChild(id, size_arg,
    // borders ? ImGuiChildFlags_Borders : ImGuiChildFlags_None, window_flags);     } // Unnecessary as true ==
    // ImGuiChildFlags_Borders
    CIMGUI_API void ImGui_ShowStackToolWindow(bool *p_open /* = NULL */);
    CIMGUI_API bool ImGui_ComboObsolete(const char *label,
                                        int *current_item,
                                        bool (*old_callback)(void *user_data, int idx, const char **out_text),
                                        void *user_data,
                                        int items_count); // Implied popup_max_height_in_items = -1
    CIMGUI_API bool ImGui_ComboObsoleteEx(const char *label,
                                          int *current_item,
                                          bool (*old_callback)(void *user_data, int idx, const char **out_text),
                                          void *user_data,
                                          int items_count,
                                          int popup_max_height_in_items /* = -1 */);
    CIMGUI_API bool ImGui_ListBoxObsolete(const char *label,
                                          int *current_item,
                                          bool (*old_callback)(void *user_data, int idx, const char **out_text),
                                          void *user_data,
                                          int items_count); // Implied height_in_items = -1
    CIMGUI_API bool ImGui_ListBoxObsoleteEx(const char *label,
                                            int *current_item,
                                            bool (*old_callback)(void *user_data, int idx, const char **out_text),
                                            void *user_data,
                                            int items_count,
    ...
    // IMGUI_API bool      ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0,
    // 0), const ImVec2& uv1 = ImVec2(1, 1), int frame_padding = -1, const ImVec4& bg_col = ImVec4(0, 0, 0, 0), const
    // ImVec4& tint_col = ImVec4(1, 1, 1, 1)); // --> Use new ImageButton() signature (explicit item id, regular
    // FramePadding). Refer to code in 1.91 if you want to grab a copy of this version.
    ...
    // >= ImGuiKey_NamedKey_BEGIN && key < ImGuiKey_NamedKey_END); const ImGuiKeyData* key_data = GetKeyData(key);
    // return (ImGuiKey)(key_data - g.IO.KeysData); } // Map ImGuiKey_* values into legacy native key index. ==
    // io.KeyMap[key]. When using a 1.87+ backend using io.AddKeyEvent(), calling GetKeyIndex() with ANY ImGuiKey_XXXX
    // values will return the same value! static inline ImGuiKey GetKeyIndex(ImGuiKey key) { IM_ASSERT(key >=
