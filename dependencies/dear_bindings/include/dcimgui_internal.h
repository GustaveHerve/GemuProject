...
struct ImVector_ImFontBakedPtr_t
{
    int Size;
    int Capacity;
    ImFontBaked **Data;
}; // Instantiation of ImVector<ImFontBaked*>
struct ImVector_ImFontAtlasPtr_t
{
    int Size;
    int Capacity;
    ImFontAtlas **Data;
...
#endif // #if (defined __SSE__ || defined __x86_64__ || defined _M_X64 ||(defined(_M_IX86_FP)&&(_M_IX86_FP >=
       // 1)))&&!defined(IMGUI_DISABLE_SSE)
...
#define IMGUI_ENABLE_STB_TRUETYPE
    ...
    // ImGui
    typedef struct ImGuiBoxSelectState_t ImGuiBoxSelectState; // Box-selection state (currently used by multi-selection,
                                                              // could potentially be used by others)
    ...
    typedef struct ImGuiDeactivatedItemData_t
    ...
    typedef struct ImGuiInputTextDeactivateData_t
        ImGuiInputTextDeactivateData; // Short term storage to backup text of a deactivating InputText() while another
    ...
    typedef struct ImGuiStyleVarInfo_t
    ...
    typedef struct ImGuiWindowTempData_t
        ImGuiWindowTempData; // Temporary storage for one window (that's the data which in theory we could ditch at the
                             // end of the frame, in practice we currently keep it for each window)
    typedef struct ImGuiWindowSettings_t
        ImGuiWindowSettings; // Storage for a window .ini settings (we keep one of those even if the actual window
...
#define ImMemchr memchr
    ...
    CIMGUI_API int
    cImStrnicmp(const char *str1, const char *str2, size_t count); // Case insensitive compare to a certain count.
    CIMGUI_API void cImStrncpy(char *dst,
                               const char *src,
    ...
    CIMGUI_API char *
    cImStrdupcpy(char *dst, size_t *p_dst_size, const char *str); // Copy in provided buffer, recreate buffer if needed.
    CIMGUI_API const char *
    cImStrchrRange(const char *str_begin, const char *str_end, char c); // Find first occurrence of 'c' in string range.
    CIMGUI_API const char *cImStreolRange(const char *str, const char *str_end); // End end-of-line
    CIMGUI_API const char *cImStristr(const char *haystack,
    ...
    CIMGUI_API void cImFormatStringToTempBuffer(const char **out_buf, const char **out_buf_end, const char *fmt, ...)
        IM_FMTARGS(3);
    CIMGUI_API void
    cImFormatStringToTempBufferV(const char **out_buf, const char **out_buf_end, const char *fmt, va_list args)
        IM_FMTLIST(3);
...
#define IMGUI_DISABLE_TTY_FUNCTIONS // Can't use stdout, fflush if we are not using default file functions
    ...
    CIMGUI_API void *cImFileLoadToMemory(const char *filename,
                                         const char *mode); // Implied out_file_size = NULL, padding_bytes = 0
    CIMGUI_API void *cImFileLoadToMemoryEx(const char *filename,
                                           const char *mode,
...
#define ImCeil(X) ceilf(X)
    CIMGUI_API float
    cImPow(float x,
           float y); // DragBehaviorT/SliderBehaviorT uses ImPow with either float/double and need the precision
    CIMGUI_API double cImPowDouble(double x, double y);
    CIMGUI_API float
    cImLog(float x); // DragBehaviorT/SliderBehaviorT uses ImLog with either float/double and need the precision
    ...
    CIMGUI_API ImVec2 cImBezierCubicClosestPoint(ImVec2 p1,
                                                 ImVec2 p2,
                                                 ImVec2 p3,
                                                 ImVec2 p4,
                                                 ImVec2 p,
                                                 int num_segments); // For curves with explicit number of segments
    CIMGUI_API ImVec2 cImBezierCubicClosestPointCasteljau(
        ImVec2 p1,
        ImVec2 p2,
        ImVec2 p3,
        ImVec2 p4,
    ...
    CIMGUI_API void
    cImTriangleBarycentricCoords(ImVec2 a, ImVec2 b, ImVec2 c, ImVec2 p, float *out_u, float *out_v, float *out_w);
    ...
    CIMGUI_API void ImRect_ClipWith(ImRect *self, ImRect r); // Simple version, may lead to an inverted rectangle, which
...
#define IM_BITARRAY_CLEARBIT(_ARRAY, _N)                                                                               \
    ...
    // Helper: ImPool<>
    // Basic keyed storage for contiguous instances, slow/amortized insertion, O(1) indexable, O(Log N) queries by ID
    // over a dense/hot buffer, Honor constructor/destructor. Add/remove invalidate all pointers. Indexes have the same
...
#define IM_DRAWLIST_ARCFAST_SAMPLE_MAX IM_DRAWLIST_ARCFAST_TABLE_SIZE // Sample index _PathArcToFastEx() for 360 angle.
    ...
    // You may want to create your own instance of you try to ImDrawList completely without ImGui. In that case, watch
    ...
    struct ImDrawListSharedData_t
    {
        ...
        ImGuiContext *Context; // [OPTIONAL] Link to Dear ImGui context. 99% of ImDrawList/ImFontAtlas can function
                               // without an ImGui context, but this facilitate handling one legacy edge case.

        // Lookup tables
        ImVec2 ArcFastVtx[IM_DRAWLIST_ARCFAST_TABLE_SIZE]; // Sample points on the quarter of the circle.
        float ArcFastRadiusCutoff;    // Cutoff radius after which arc drawing will fallback to slower PathArcTo()
        ImU8 CircleSegmentCounts[64]; // Precomputed segment count for given radius before we calculate it dynamically
    ...
    struct ImVector_ImFontStackData_t
    {
        int Size;
        int Capacity;
    ...
    // - input: PushItemFlag() manipulates g.CurrentItemFlags, g.NextItemData.ItemFlags, ItemAdd() calls may add extra
    // flags too.
    ...
    typedef enum
    {
        ...
        ImGuiItemFlags_MixedValue =
            1
            << 12, // false     // [BETA] Represent a mixed/indeterminate value, generally multi-selection where values
        ...
        ImGuiItemFlags_NoFocus =
            1 << 17, // false     // [EXPERIMENTAL: Not very well specced] Clicking doesn't take focus. Automatically
                     // sets ImGuiButtonFlags_NoFocus + ImGuiButtonFlags_NoNavFocus in ButtonBehavior().

        // Controlled by widget code
        ImGuiItemFlags_Inputable =
            1 << 20, // false     // [WIP] Auto-activate input mode when tab focused. Currently only used and supported
    ...
    typedef enum
    {
        ...
        ImGuiItemStatusFlags_HoveredRect =
            1 << 0, // Mouse position is within item rectangle (does NOT mean that the window is in correct z-order and
                    // can be hovered!, this is only one part of the most-common IsItemHovered test)
        ...
        ImGuiItemStatusFlags_Visible =
            1 << 8, // [WIP] Set when item is overlapping the current clipping rectangle (Used internally. Please don't
    ...
    typedef enum
    {
        ...
        ImGuiButtonFlags_NoNavFocus =
            1 << 18, // don't override navigation focus when activated (FIXME: this is essentially used every time an
                     // item uses ImGuiItemFlags_NoNav, but because legacy specs don't requires LastItemData to be set
        ...
        ImGuiButtonFlags_NoTestKeyOwner = 1
                                          << 21, // don't test key/input owner when polling the key (note: mouse buttons
                                                 // are keys! often, the key in question will be ImGuiKey_MouseLeft!)
    ...
    typedef enum
    {
        ...
        ImGuiSelectableFlags_SelectOnNav =
            1 << 21, // (WIP) Auto-select when moved into. This is not exposed in public API as to handle multi-select
                     // and modifiers we will need user to explicitly control focus scope. May be replaced with a
                     // BeginSelection() API.
        ...
        ImGuiSelectableFlags_SpanAvailWidth =
            1 << 24, // Span all avail width even if we declared less for layout purpose. FIXME: We may be able to
                     // remove this (added in 6251d379, 2bcafc86 for menus)
        ImGuiSelectableFlags_SetNavIdOnHover = 1 << 25,      // Set Nav/Focus ID on mouse hover (used by MenuItem)
        ImGuiSelectableFlags_NoPadWithHalfSpacing = 1 << 26, // Disable padding each side with ItemSpacing * 0.5f
        ImGuiSelectableFlags_NoSetKeyOwner =
            1 << 27, // Don't set key/input owner on the initial click (note: mouse buttons are keys! often, the key in
    ...
    // Simple column measurement, currently used for MenuItem() only.. This is very short-sighted/throw-away code and
...
#define IMSTB_TEXTEDIT_UNDOCHARCOUNT 999
    ...
    struct ImGuiInputTextState_t
    {
        ...
        const char *TextSrc; // == TextA.Data unless read-only, in which case == buf passed to InputText(). Field only
                             // set and valid _inside_ the call InputText() call.
        ImVector_char TextA; // main UTF8 buffer. TextA.Size is a buffer size! Should always be >= buf_size passed by
                             // user (and of course >= CurLenA + 1).
        ImVector_char TextToRevertTo; // value to revert to when pressing Escape = backup of end-user buffer at the time
        ...
        ImVec2 Scroll;     // horizontal offset (managed manually) + vertical scrolling (pulled from child window's own
                           // Scroll.y)
        ...
        bool WantReloadUserBuf;    // force a reload of user buf so it may be modified externally. may be automatic in
                                   // future version.
    ...
    typedef enum
    {
        ...
        ImGuiWindowRefreshFlags_TryToAvoidRefresh = 1 << 0, // [EXPERIMENTAL] Try to keep existing contents, USER MUST
                                                            // NOT HONOR BEGIN() RETURNING FALSE AND NOT APPEND.
    ...
    struct ImGuiNextWindowData_t
    {
        ...
        ImVec2 MenuBarOffsetMinVal; // (Always on) This is not exposed publicly, so we don't clear it and it doesn't
                                    // have a corresponding flag (could we? for consistency?)
    ...
    struct ImGuiNextItemData_t
    {
        ...
        // Members below are NOT cleared by ItemAdd() meaning they are still valid during e.g. NavProcessItem(). Always
        // rely on HasFlags.
    ...
    struct ImGuiLastItemData_t
    {
        ...
        // Rarely used fields are not explicitly cleared, only valid when the corresponding ImGuiItemStatusFlags are
        // set.
        ImRect
    ...
    {
        void *Ptr; // Either field can be set, not both. e.g. Dock node tab bars are loose while BeginTabBar() ones are
                   // in a pool.
    ...
    struct ImGuiPopupData_t
    {
        ...
        int ParentNavLayer;            // Resolved on BeginPopup(). Actually a ImGuiNavLayer type (declared down below),
                            // initialized to -1 which is not part of an enum, but serves well-enough as "not any of
                            // layers" value
...
#define ImGuiKeyOwner_NoOwner ((ImGuiID) - 1) // Require key to have no owner.
    ...
    // Routing table: maintain a desired owner for each possible key-chord (key + mods), and setup owner in NewFrame()
    // when mods are matching. Stored in main context (1 instance)
    ...
    struct ImGuiKeyOwnerData_t
    {
        ...
        bool LockUntilRelease; // Reading this key requires explicit owner id (until key is released). Set by
                               // ImGuiInputFlags_LockUntilRelease. When this is true LockThisFrame is always true as
                               // well.
    };
    ...
    typedef enum
    {
        ...
        ImGuiInputFlags_RepeatUntilRelease =
            1 << 4, // Stop repeating when released (default for all functions except Shortcut). This only exists to
        ...
        // - Locking key away from non-input aware code. Locking is useful to make input-owner-aware code steal keys
        // from non-input-owner-aware code. If all code is input-owner-aware locking would never be necessary.
        ImGuiInputFlags_LockThisFrame =
        ...
        ImGuiInputFlags_LockUntilRelease =
            1
            << 21, // Further accesses to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT accepted
    ...
    typedef enum
    {
        ...
        ImGuiScrollFlags_KeepVisibleEdgeY =
            1 << 1, // If item is not visible: scroll as little as possible on Y axis to bring item back into view
    ...
    typedef enum
    {
        ...
        ImGuiNavMoveFlags_FocusApi =
            1 << 9, // Requests from focus API can land/focus/activate items even if they are marked with _NoTabStop
    ...
    struct ImGuiNavItemData_t
    {
        ...
        ImGuiSelectionUserData SelectionUserData; // I+Mov    // Best candidate SetNextItemSelectionUserData() value.
                                                  // Valid if (ItemFlags & ImGuiItemFlags_HasSelectionUserData)
    };
    ...
    typedef enum
    {
        ...
        ImGuiTypingSelectFlags_AllowBackspace =
            1 << 0, // Backspace to delete character inputs. If using: ensure GetTypingSelectRequest() is not called
    ...
    struct ImGuiTypingSelectRequest_t
    {
        ...
        int SearchBufferLen;
        const char *SearchBuffer; // Search buffer contents (use full string. unless SingleCharMode is set, in which
    ...
    struct ImGuiTypingSelectState_t
    {
        ...
        bool SingleCharModeLock /* = false */; // After a certain single char repeat count we lock into SingleCharMode.
                                               // Two benefits: 1) buffer never fill, 2) we can provide an immediate
                                               // SingleChar mode without timer elapsing.
    };
    CIMGUI_API void
    ImGuiTypingSelectState_Clear(ImGuiTypingSelectState *self); // We preserve remaining data for easier debugging
...
#define ImGuiSelectionUserData_Invalid ((ImGuiSelectionUserData) - 1)
    ...
    CIMGUI_API void ImGuiMultiSelectTempData_Clear(
        ImGuiMultiSelectTempData *self); // Zero-clear except IO as we preserve IO.Requests[] buffer allocation.
...
#define IM_ASSERT_USER_ERROR(_EXPR, _MSG)                                                                              \
    ...
    struct ImGuiContext_t
    {
        ...
        ImGuiStyle Style;
        ImVector_ImFontAtlasPtr FontAtlases; // List of font atlases used by the context (generally only contains
        ...
        bool WithinFrameScopeWithImplicitWindow; // Set by NewFrame(), cleared by EndFrame() when the implicit debug
        ...
        ImVector_ImGuiInputEvent InputEventsTrail; // Past input events processed in NewFrame(). This is to allow
        ...
        ImVector_ImGuiWindowPtr WindowsTempSortBuffer; // Temporary buffer used in EndFrame() to reorder windows so
                                                       // parents are kept before their child
        ImVector_ImGuiWindowStackData CurrentWindowStack;
        ImGuiStorage WindowsById;        // Map window's ImGuiID to ImGuiWindow*
        int WindowsActiveCount;          // Number of unique windows submitted by frame
        float WindowsBorderHoverPadding; // Padding around resizable windows for which hovering on counts as hovering
                                         // the window == ImMax(style.TouchExtraPadding,
        ...
        ImGuiWindow *MovingWindow;   // Track the window we clicked on (in order to preserve focus). The actual window
                                     // that is moved is generally MovingWindow->RootWindow.
        ...
        bool HoveredIdIsDisabled; // At least one widget passed the rect test, but has been discarded by disabled flag
        ...
        ImGuiID ActiveIdIsAlive; // Active widget has been seen this frame (we can't use a bool as the ActiveId may
        ...
        bool ActiveIdHasBeenPressedBefore; // Track whether the active id led to a press (this is to allow changing
                                           // between PressOnClick and PressOnRelease without pressing twice). Used by
                                           // range_select branch.
        ...
        ImGuiDataTypeStorage ActiveIdValueOnActivation; // Backup of initial value at the time of activation. ONLY SET
                                                        // BY SPECIFIC WIDGETS: DragXXX and SliderXXX.
        ...
        bool ActiveIdUsingAllKeyboardKeys; // Active widget will want to read all keyboard keys inputs. (this is a
                                           // shortcut for not taking ownership of 100+ keys, frequently used by drag
                                           // operations)
        ...
        ImGuiID CurrentFocusScopeId;     // Value for currently appending items == g.FocusScopeStack.back(). Not to be
                                         // mistaken with g.NavFocusScopeId.
        ImGuiItemFlags CurrentItemFlags; // Value for currently appending items == g.ItemFlagsStack.back()
        ImGuiID DebugLocateId; // Storage for DebugLocateItemOnHover() feature: this is read by ItemAdd() so we keep it
                               // in a hot/cached location
        ...
        ImVector_ImGuiFocusScopeData FocusScopeStack;  // Stack for PushFocusScope()/PopFocusScope() - inherited by
        ...
        bool NavCursorVisible; // Nav focus cursor/rectangle is visible? We hide it after a mouse click. We show it
                               // after a nav move.
        ...
        bool NavMousePosDirty;   // When set we will update mouse position if io.ConfigNavMoveSetMousePos is set (not
                                 // enabled by default)
        ...
        // ImGuiID               NavActivateInputId;                 // Removed in 1.89.4 (July 2023). This is now part
        // of g.NavActivateId and sets g.NavActivateFlags |= ImGuiActivateFlags_PreferInput. See commit c9a53aa74, issue
        // #5606.
        ...
        ImRect NavScoringRect;   // Rectangle used for scoring, in screen space. Based of window->NavRectRel[], modified
                                 // for directional navigation scoring.
        ImRect NavScoringNoClipRect; // Some nav operations (such as PageUp/PageDown) enforce a region which clipper
                                     // will attempt to always keep submitted
        int NavScoringDebugCount;    // Metrics for debugging
        int NavTabbingDir;           // Generally -1 or +1, 0 when tabbing without a nav id
        int NavTabbingCounter;       // >0 when counting items for tabbing
        ImGuiNavItemData NavMoveResultLocal;        // Best move request candidate within NavWindow
        ImGuiNavItemData NavMoveResultLocalVisible; // Best move request candidate within NavWindow that are mostly
        ...
        bool ConfigNavWindowingWithGamepad; // = true. Enable CTRL+TAB by holding ImGuiKey_GamepadFaceLeft (==
                                            // ImGuiKey_NavGamepadMenu). When false, the button may still be used to
                                            // toggle Menu layer.
        ImGuiKeyChord ConfigNavWindowingKeyNext; // = ImGuiMod_Ctrl | ImGuiKey_Tab (or ImGuiMod_Super | ImGuiKey_Tab on
                                                 // OS X). For reconfiguration (see #4828)
        ...
        ImGuiWindow *NavWindowingTarget;     // Target window when doing CTRL+Tab (or Pad Menu + FocusPrev/Next), this
        ...
        float DragDropAcceptIdCurrRectSurface; // Target item surface (we resolve overlapping targets by prioritizing
                                               // the smaller surface)
        ...
        ImGuiPlatformImeData
            PlatformImeData; // Data updated by current frame. Will be applied at end of the frame. For some backends,
                             // this is required to have WantVisible=true in order to receive text message.
        ...
        ImVector_ImTextureDataPtr UserTextures; // List of textures created/managed by user or third-party extension.
       ...
       // Misc
        ...
        int WantCaptureMouseNextFrame; // Explicit capture override via
                                       // SetNextFrameWantCaptureMouse()/SetNextFrameWantCaptureKeyboard(). Default to
                                       // -1.
        int WantCaptureKeyboardNextFrame; // "
        int WantTextInputNextFrame; // Copied in EndFrame() from g.PlatformImeData.WantTextInput. Needs to be set for
                                    // some backends (SDL3) to emit character inputs.
    ...
    // Transient per-window data, reset at the beginning of the frame. This used to be called ImGuiDrawContext, hence
    // the DC variable name in ImGuiWindow. (That's theory, in practice the delimitation between ImGuiWindow and
    ...
    struct ImGuiWindowTempData_t
    {
        ...
        ImVec1 ColumnsOffset; // Offset to the current column (if ColumnsCurrent > 0). FIXME: This and the above should
                              // be a stack to allow use cases like Tree->Column->Tree. Need revamp columns API.
        ImVec1 GroupOffset;
        ImVec2 CursorStartPosLossyness; // Record the loss of precision of CursorStartPos due to really large scrolling
                                        // amount. This is used by clipper to compensate and fix the most common use
        ...
        bool NavIsScrollPushableX;     // Set when current work location may be scrolled horizontally when moving left /
                                       // right. This is generally always true UNLESS within a column.
        ...
        ImU32 TreeHasStackDataDepthMask; // Store whether given depth has ImGuiTreeNodeStackData data. Could be turned
                                         // into a ImU64 if necessary.
        ImU32 TreeRecordsClippedNodesY2Mask; // Store whether we should keep recording Y2. Cleared when passing clip
                                             // max. Equivalent TreeHasStackDataDepthMask value should always be set.
        ...
        // vectors are rarely modified. Also it allows us to not heap allocate for short-lived windows which are not
    ...
    struct ImGuiWindow_t
    {
        ...
        float TitleBarHeight, MenuBarHeight; // Note that those used to be function before 2024/05/28. If you have old
        ...
        ImGuiID ChildId; // ID of corresponding item in parent window (for navigation to return from child window to
                         // parent window)
        ImGuiID PopupId; // ID in the popup stack when this window is used as a popup/menu (because we use generic
                         // Name/ID for recycling)
        ImVec2 Scroll;
        ImVec2 ScrollMax;
        ...
        ImVector_ImGuiID IDStack; // ID stack. ID are hashes seeded with the value at the top of the stack. (In theory
                                  // this should be in the TempData structure)
        ImGuiWindowTempData DC;   // Temporary per-window data, reset at the beginning of the frame. This used to be
                                  // called ImGuiDrawContext, hence the "DC" variable name.
        ...
        ImRect InnerClipRect;    // == InnerRect shrunk by WindowPadding*0.5f on each side, clipped within viewport or
                                 // parent clip rect.
        ImRect WorkRect; // Initially covers the whole scrolling region. Reduced by containers e.g columns/tables when
                         // active. Shrunk by WindowPadding*1.0f on each side. This is meant to replace
        ...
        int SettingsOffset; // Offset into SettingsWindows[] (offsets are always valid as we only grow the array from
                            // the back)
        ...
        ImGuiWindow *NavLastChildNavWindow; // When going to the menu bar, we remember the child window we came from.
                                            // (This could probably be made implicit if we kept g.Windows sorted by last
        ...
        int MemoryDrawListIdxCapacity; // Backup of last idx/vtx count, so when waking up the window we can preallocate
    ...
    typedef enum
    {
        ...
        ImGuiTabItemFlags_NoCloseButton = 1 << 20, // Track whether p_open was set or not (we'll need this info on the
        ...
        // ImGuiTabItemFlags_Unsorted                = 1 << 23,  // [Docking] Trailing tabs with the _Unsorted flag will
    ...
    struct ImGuiTabBar_t
    {
        ...
        ImRect BarRect;
        float BarRectPrevWidth; // Backup of previous width. When width change we enforce keep horizontal scroll on
                                // focused tab.
...
#define IMGUI_TABLE_MAX_COLUMNS                                                                                        \
    ...
    // This is in contrast with some user-facing api such as IsItemVisible() / IsRectVisible() which use "Visible" to
    // mean "not clipped".
    struct ImGuiTableColumn_t
    {
        ...
        float WidthGiven; // Final/actual width visible == (MaxX - MinX), locked in TableUpdateLayout(). May be >
                          // WidthRequest to honor minimum width, may be < WidthRequest to honor shrinking columns down
                          // in tight space.
        ...
        float ContentMaxXFrozen; // Contents maximum position for frozen rows (apart from headers), from which we can
                                 // infer content width.
        float ContentMaxXUnfrozen;
        float ContentMaxXHeadersUsed; // Contents maximum position for headers rows (regardless of freezing).
                                      // TableHeader() automatically softclip itself + report ideal desired size, to
        ...
        ImGuiTableColumnIdx NextEnabledColumn;     // Index of next enabled/visible column within Columns[], -1 if last
                                                   // enabled/visible column
        ImGuiTableColumnIdx SortOrder; // Index of this column within sort specs, -1 if not sorting on this column, 0
                                       // for single-sort, may be >0 on multi-sort
        ImGuiTableDrawChannelIdx DrawChannelCurrent;  // Index within DrawSplitter.Channels[]
        ImGuiTableDrawChannelIdx DrawChannelFrozen;   // Draw channels for frozen rows (often headers)
        ImGuiTableDrawChannelIdx DrawChannelUnfrozen; // Draw channels for unfrozen rows
        bool IsEnabled;                               // IsUserEnabled && (Flags & ImGuiTableColumnFlags_Disabled) == 0
        bool IsUserEnabled; // Is the column not marked Hidden by the user? (unrelated to being off view, e.g. clipped
                            // by scrolling).
    ...
    // Per-instance data that needs preserving across frames (seemingly most others do not need to be preserved aside
    // from debug needs. Does that means they could be moved to ImGuiTableTempData?) sizeof() ~ 24 bytes
    ...
    struct ImGuiTable_t
    {
        ...
        ImSpan_ImGuiTableColumnIdx DisplayOrderToIndex; // Point within RawData[]. Store display order of columns (when
        ...
        ImS16 InstanceCurrent;    // Count of BeginTable() calls with same ID in the same frame (generally 0). This is a
                                  // little bit similar to BeginCount for a window, but multiple tables with the same ID
        ...
        float ColumnsAutoFitWidth; // Sum of ideal column width in order nothing to be clipped, used for auto-fitting
                                   // and content width submission in outer window
        ...
        ImRect
            Bg2ClipRectForDrawCmd; // Actual ImDrawCmd clip rect for BG2 channel. This tends to be a correct, tight-fit,
        ...
        ImGuiTableDrawChannelIdx Bg2DrawChannelCurrent; // For Selectable() and other widgets drawing across columns
                                                        // after the freezing line. Index within DrawSplitter.Channels[]
    ...
    // stacked table).
    // - Accessing those requires chasing an extra pointer so for very frequently used data we leave them in the main
    // table structure.
    ...
    struct ImGuiTableSettings_t
    {
        ...
        ImGuiTableFlags SaveFlags; // Indicate data we want to save using the Resizable/Reorderable/Sortable/Hideable
    ...
    CIMGUI_API void
    ImGui_UpdateWindowParentAndRootLinks(ImGuiWindow *window, ImGuiWindowFlags flags, ImGuiWindow *parent_window);
    ...
    CIMGUI_API void
    ImGui_SetWindowCollapsedImGuiWindowPtr(ImGuiWindow *window, bool collapsed, ImGuiCond cond /* = 0 */);
    ...
    CIMGUI_API void ImGui_FocusTopMostWindowUnderOne(ImGuiWindow *under_this_window,
    ...
    // Fonts, drawing
    CIMGUI_API void
    ImGui_RegisterUserTexture(ImTextureData *tex); // Register external texture. EXPERIMENTAL: DO NOT USE YET.
    ...
    CIMGUI_API ImDrawList *ImGui_GetForegroundDrawListImGuiWindowPtr(
        ImGuiWindow *window); // This seemingly unnecessary wrapper simplifies compatibility between the 'master' and
                              // 'docking' branches.
    CIMGUI_API ImDrawList *ImGui_GetBackgroundDrawListImGuiViewportPtr(
    ...
    CIMGUI_API ImDrawList *ImGui_GetForegroundDrawListImGuiViewportPtr(
    ...
    CIMGUI_API void
    ImGui_AddDrawListToDrawDataEx(ImDrawData *draw_data, ImVector_ImDrawListPtr *out_list, ImDrawList *draw_list);

    // Init
    CIMGUI_API void ImGui_Initialize(void);
    CIMGUI_API void ImGui_Shutdown(void); // Since 1.60 this is a _private_ function. You can call DestroyContext() to
    ...
    CIMGUI_API void ImGui_FindHoveredWindowEx(ImVec2 pos,
    ...
    CIMGUI_API void ImGui_ItemSizeImRectEx(
        ImRect bb,
        float text_baseline_y /* = -1.0f */); // FIXME: This is a misleading API since we expect CursorPos to be bb.Min.
    CIMGUI_API bool ImGui_ItemAdd(ImRect bb, ImGuiID id); // Implied nav_bb = NULL, extra_flags = 0
    CIMGUI_API bool
    ImGui_ItemAddEx(ImRect bb, ImGuiID id, const ImRect *nav_bb /* = NULL */, ImGuiItemFlags extra_flags /* = 0 */);
    ...
    CIMGUI_API void ImGui_SetLastItemData(ImGuiID item_id,
    ...
    // Logging/Capture
    CIMGUI_API void ImGui_LogBegin(ImGuiLogFlags flags,
    ...
    // Childs
    CIMGUI_API bool ImGui_BeginChildEx(
        const char *name, ImGuiID id, ImVec2 size_arg, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags);
    ...
    CIMGUI_API ImVec2 ImGui_FindBestWindowPosForPopupEx(ImVec2 ref_pos,
                                                        ImVec2 size,
                                                        ImGuiDir *last_dir,
                                                        ImRect r_outer,
                                                        ImRect r_avoid,
    ...
    // Menus
    CIMGUI_API bool ImGui_BeginViewportSideBar(
    ...
    CIMGUI_API bool
    ImGui_MenuItemWithIcon(const char *label,
                           const char *icon); // Implied shortcut = NULL, selected = false, enabled = true
    CIMGUI_API bool ImGui_MenuItemWithIconEx(const char *label,
                                             const char *icon,
    ...
    CIMGUI_API void ImGui_NavMoveRequestSubmit(ImGuiDir move_dir,
                                               ImGuiDir clip_dir,
                                               ImGuiNavMoveFlags move_flags,
                                               ImGuiScrollFlags scroll_flags);
    CIMGUI_API void ImGui_NavMoveRequestForward(ImGuiDir move_dir,
                                                ImGuiDir clip_dir,
                                                ImGuiNavMoveFlags move_flags,
                                                ImGuiScrollFlags scroll_flags);
    CIMGUI_API void ImGui_NavMoveRequestResolveWithLastItem(ImGuiNavItemData *result);
    CIMGUI_API void ImGui_NavMoveRequestResolveWithPastTreeNode(ImGuiNavItemData *result,
    ...
    CIMGUI_API void ImGui_ActivateItemByID(
    ...
    CIMGUI_API ImVec2 ImGui_GetKeyMagnitude2d(ImGuiKey key_left,
                                              ImGuiKey key_right,
                                              ImGuiKey key_up,
                                              ImGuiKey key_down);
    ...
    CIMGUI_API void ImGui_SetItemKeyOwnerImGuiInputFlags(
        ImGuiKey key,
    ...
    CIMGUI_API bool
    ImGui_TestKeyOwner(ImGuiKey key, ImGuiID owner_id); // Test that key is either not owned, either owned by 'owner_id'
    ...
    CIMGUI_API bool ImGui_IsKeyPressedImGuiInputFlagsEx(
        ImGuiKey key,
        ImGuiInputFlags flags,
        ImGuiID owner_id /* = 0 */); // Important: when transitioning from old to new IsKeyPressed(): old API has "bool
    ...
    CIMGUI_API bool ImGui_IsKeyChordPressedImGuiInputFlags(ImGuiKeyChord key_chord,
                                                           ImGuiInputFlags flags); // Implied owner_id = 0
    CIMGUI_API bool ImGui_IsKeyChordPressedImGuiInputFlagsEx(ImGuiKeyChord key_chord,
                                                             ImGuiInputFlags flags,
                                                             ImGuiID owner_id /* = 0 */);
    CIMGUI_API bool ImGui_IsMouseDownID(ImGuiMouseButton button, ImGuiID owner_id);
    CIMGUI_API bool ImGui_IsMouseClickedImGuiInputFlags(ImGuiMouseButton button,
                                                        ImGuiInputFlags flags); // Implied owner_id = 0
    CIMGUI_API bool
    ImGui_IsMouseClickedImGuiInputFlagsEx(ImGuiMouseButton button, ImGuiInputFlags flags, ImGuiID owner_id /* = 0 */);
    ...
    CIMGUI_API bool ImGui_SetShortcutRouting(ImGuiKeyChord key_chord,
                                             ImGuiInputFlags flags,
    ...
    ImGui_GetTypingSelectRequest(void); // Implied flags = ImGuiTypingSelectFlags_None
    CIMGUI_API ImGuiTypingSelectRequest *
    ImGui_GetTypingSelectRequestEx(ImGuiTypingSelectFlags flags /* = ImGuiTypingSelectFlags_None */);
    CIMGUI_API int ImGui_TypingSelectFindMatch(ImGuiTypingSelectRequest *req,
                                               int items_count,
                                               const char *(*get_item_name_func)(void *, int),
                                               void *user_data,
                                               int nav_item_idx);
    CIMGUI_API int ImGui_TypingSelectFindNextSingleCharMatch(ImGuiTypingSelectRequest *req,
                                                             int items_count,
                                                             const char *(*get_item_name_func)(void *, int),
                                                             void *user_data,
                                                             int nav_item_idx);
    CIMGUI_API int ImGui_TypingSelectFindBestLeadingMatch(ImGuiTypingSelectRequest *req,
                                                          int items_count,
                                                          const char *(*get_item_name_func)(void *, int),
                                                          void *user_data);
    ...
    // Box-Select API
    CIMGUI_API bool
    ImGui_BeginBoxSelect(ImRect scope_rect, ImGuiWindow *window, ImGuiID box_select_id, ImGuiMultiSelectFlags ms_flags);
    ...
    CIMGUI_API void ImGui_MultiSelectAddSetRange(ImGuiMultiSelectTempData *ms,
                                                 bool selected,
                                                 int range_dir,
    ...
    CIMGUI_API void ImGui_BeginColumns(
        const char *str_id,
        int count,
    ...
    CIMGUI_API void
    ImGui_TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_direction, bool append_to_sort_specs);
    CIMGUI_API int ImGui_TableGetHoveredRow(void); // Retrieve *PREVIOUS FRAME* hovered row. This difference with
    ...
    CIMGUI_API void ImGui_TableAngledHeadersRowEx(
    ...
    CIMGUI_API bool
    ImGui_BeginTableWithID(const char *name,
                           ImGuiID id,
                           int columns_count,
                           ImGuiTableFlags flags /* = 0 */); // Implied outer_size = ImVec2(0, 0), inner_width = 0.0f
    CIMGUI_API bool ImGui_BeginTableWithIDEx(const char *name,
                                             ImGuiID id,
                                             int columns_count,
    ...
    CIMGUI_API bool ImGui_TabItemEx(
    ...
    CIMGUI_API void ImGui_TabItemLabelAndCloseButton(ImDrawList *draw_list,
                                                     ImRect bb,
                                                     ImGuiTabItemFlags flags,
    ...
    CIMGUI_API void ImGui_RenderText(ImVec2 pos,
                                     const char *text); // Implied text_end = NULL, hide_text_after_hash = true
    CIMGUI_API void ImGui_RenderTextEx(ImVec2 pos,
                                       const char *text,
    ...
    CIMGUI_API void
    ImGui_RenderTextClipped(ImVec2 pos_min,
                            ImVec2 pos_max,
                            const char *text,
                            const char *text_end,
                            const ImVec2 *text_size_if_known); // Implied align = ImVec2(0, 0), clip_rect = NULL
    CIMGUI_API void ImGui_RenderTextClippedEx(ImVec2 pos_min,
                                              ImVec2 pos_max,
                                              const char *text,
                                              const char *text_end,
                                              const ImVec2 *text_size_if_known,
                                              ImVec2 align /* = ImVec2(0, 0) */,
                                              const ImRect *clip_rect /* = NULL */);
    CIMGUI_API void ImGui_RenderTextClippedWithDrawList(
        ...
        const char *text_end,
        const ImVec2 *text_size_if_known); // Implied align = ImVec2(0, 0), clip_rect = NULL
    CIMGUI_API void ImGui_RenderTextClippedWithDrawListEx(ImDrawList *draw_list,
                                                          ImVec2 pos_min,
                                                          ImVec2 pos_max,
                                                          const char *text,
                                                          const char *text_end,
                                                          ...
                                                          ImVec2 align /* = ImVec2(0, 0) */,
                                                          const ImRect *clip_rect /* = NULL */);
    CIMGUI_API void ImGui_RenderTextEllipsis(ImDrawList *draw_list,
                                             ImVec2 pos_min,
                                             ImVec2 pos_max,
                                             ...
                                             const ImVec2 *text_size_if_known);
    CIMGUI_API void
    ImGui_RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col); // Implied borders = true, rounding = 0.0f
    CIMGUI_API void ImGui_RenderFrameEx(
    ...
    CIMGUI_API void ImGui_RenderColorRectWithAlphaCheckerboard(ImDrawList *draw_list,
                                                               ImVec2 p_min,
                                                               ImVec2 p_max,
                                                               ImU32 fill_col,
                                                               float grid_step,
                                                               ImVec2 grid_off); // Implied rounding = 0.0f, flags = 0
    CIMGUI_API void ImGui_RenderColorRectWithAlphaCheckerboardEx(ImDrawList *draw_list,
                                                                 ImVec2 p_min,
                                                                 ImVec2 p_max,
                                                                 ImU32 fill_col,
                                                                 float grid_step,
    ...
    CIMGUI_API void ImGui_RenderNavCursorEx(
        ImRect bb,
        ImGuiID id,
    ...
    CIMGUI_API void ImGui_RenderNavHighlightEx(
        ImRect bb,
        ImGuiID id,
        ImGuiNavRenderCursorFlags flags /* = ImGuiNavRenderCursorFlags_None */); // Renamed in 1.91.4
#endif                                                                  // #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    CIMGUI_API const char *ImGui_FindRenderedTextEnd(const char *text); // Implied text_end = NULL
    CIMGUI_API const char *ImGui_FindRenderedTextEndEx(
        const char *text,
        const char *text_end /* = NULL */); // Find the optional ## from which we stop displaying text.
    CIMGUI_API void ImGui_RenderMouseCursor(
        ImVec2 pos, float scale, ImGuiMouseCursor mouse_cursor, ImU32 col_fill, ImU32 col_border, ImU32 col_shadow);

    // Render helpers (those functions don't access any ImGui state!)
    CIMGUI_API void
    ImGui_RenderArrow(ImDrawList *draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir); // Implied scale = 1.0f
    CIMGUI_API void
    ImGui_RenderArrowEx(ImDrawList *draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir, float scale /* = 1.0f */);
    ...
    CIMGUI_API void
    ImGui_RenderArrowPointingAt(ImDrawList *draw_list, ImVec2 pos, ImVec2 half_sz, ImGuiDir direction, ImU32 col);
    CIMGUI_API void ImGui_RenderRectFilledRangeH(
        ImDrawList *draw_list, ImRect rect, ImU32 col, float x_start_norm, float x_end_norm, float rounding);
    CIMGUI_API void
    ImGui_RenderRectFilledWithHole(ImDrawList *draw_list, ImRect outer, ImRect inner, ImU32 col, float rounding);
    ...
    CIMGUI_API void ImGui_TextAligned(
        float align_x,
        float size_x,
        const char *fmt,
    ...
    CIMGUI_API bool
    ImGui_ButtonWithFlagsEx(const char *label, ImVec2 size_arg /* = ImVec2(0, 0) */, ImGuiButtonFlags flags /* = 0 */);
    CIMGUI_API bool
    ImGui_ArrowButtonEx(const char *str_id, ImGuiDir dir, ImVec2 size_arg, ImGuiButtonFlags flags /* = 0 */);
    CIMGUI_API bool ImGui_ImageButtonWithFlags(ImGuiID id,
    ...
    CIMGUI_API bool ImGui_ScrollbarEx(ImRect bb,
                                      ImGuiID id,
                                      ImGuiAxis axis,
                                      ImS64 *p_scroll_v,
                                      ImS64 avail_v,
                                      ImS64 contents_v); // Implied draw_rounding_flags = 0
    CIMGUI_API bool ImGui_ScrollbarExEx(ImRect bb,
                                        ImGuiID id,
                                        ImGuiAxis axis,
                                        ImS64 *p_scroll_v,
                                        ImS64 avail_v,
    ...
    CIMGUI_API bool
    ImGui_ButtonBehavior(ImRect bb, ImGuiID id, bool *out_hovered, bool *out_held, ImGuiButtonFlags flags /* = 0 */);
    CIMGUI_API bool ImGui_DragBehavior(ImGuiID id,
    ...
    CIMGUI_API bool ImGui_SliderBehavior(ImRect bb,
                                         ImGuiID id,
                                         ImGuiDataType data_type,
                                         void *p_v,
                                         const void *p_min,
                                         ...
                                         ImRect *out_grab_bb);
    CIMGUI_API bool
    ImGui_SplitterBehavior(ImRect bb,
                           ImGuiID id,
                           ImGuiAxis axis,
                           float *size1,
                           float *size2,
                           float min_size1,
                           float min_size2); // Implied hover_extend = 0.0f, hover_visibility_delay = 0.0f, bg_col = 0
    CIMGUI_API bool ImGui_SplitterBehaviorEx(ImRect bb,
                                             ImGuiID id,
                                             ImGuiAxis axis,
                                             float *size1,
                                             float *size2,
    ...
    // Widgets: Tree Nodes
    CIMGUI_API bool
    ImGui_TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label); // Implied label_end = NULL
    CIMGUI_API bool ImGui_TreeNodeBehaviorEx(ImGuiID id,
    ...
    CIMGUI_API bool
    ImGui_TreeNodeUpdateNextOpen(ImGuiID storage_id,
    ...
    CIMGUI_API int ImGui_DataTypeFormatString(
        char *buf, int buf_size, ImGuiDataType data_type, const void *p_data, const char *format);
    CIMGUI_API void
    ImGui_DataTypeApplyOp(ImGuiDataType data_type, int op, void *output, const void *arg_1, const void *arg_2);
    CIMGUI_API bool ImGui_DataTypeApplyFromText(const char *buf,
                                                ImGuiDataType data_type,
                                                void *p_data,
                                                const char *format); // Implied p_data_when_empty = NULL
    CIMGUI_API bool ImGui_DataTypeApplyFromTextEx(const char *buf,
                                                  ImGuiDataType data_type,
                                                  void *p_data,
                                                  const char *format,
    ...
    // InputText
    CIMGUI_API bool
    ImGui_InputTextWithHintAndSize(const char *label,
                                   const char *hint,
                                   char *buf,
                                   int buf_size,
                                   ImVec2 size_arg,
                                   ImGuiInputTextFlags flags); // Implied callback = NULL, user_data = NULL
    CIMGUI_API bool ImGui_InputTextWithHintAndSizeEx(const char *label,
                                                     const char *hint,
                                                     char *buf,
                                                     int buf_size,
                                                     ImVec2 size_arg,
    ...
    CIMGUI_API bool
    ImGui_TempInputText(ImRect bb, ImGuiID id, const char *label, char *buf, int buf_size, ImGuiInputTextFlags flags);
    CIMGUI_API bool ImGui_TempInputScalar(ImRect bb,
                                          ImGuiID id,
                                          const char *label,
                                          ImGuiDataType data_type,
                                          void *p_data,
                                          const char *format); // Implied p_clamp_min = NULL, p_clamp_max = NULL
    CIMGUI_API bool ImGui_TempInputScalarEx(ImRect bb,
                                            ImGuiID id,
                                            const char *label,
                                            ImGuiDataType data_type,
                                            void *p_data,
    ...
    CIMGUI_API bool
    ImGui_IsItemActiveAsInputText(void); // This may be useful to apply workaround that a based on distinguish whenever
    ...
    // Plot
    CIMGUI_API int ImGui_PlotEx(ImGuiPlotType plot_type,
                                const char *label,
    ...
    CIMGUI_API void ImGui_ShadeVertsLinearColorGradientKeepAlpha(ImDrawList *draw_list,
                                                                 int vert_start_idx,
                                                                 int vert_end_idx,
                                                                 ImVec2 gradient_p0,
                                                                 ImVec2 gradient_p1,
                                                                 ImU32 col0,
                                                                 ImU32 col1);
    CIMGUI_API void ImGui_ShadeVertsLinearUV(ImDrawList *draw_list,
                                             int vert_start_idx,
                                             int vert_end_idx,
                                             ImVec2 a,
                                             ImVec2 b,
                                             ...
                                             ImVec2 uv_b,
                                             bool clamp);
    CIMGUI_API void ImGui_ShadeVertsTransformPos(ImDrawList *draw_list,
                                                 int vert_start_idx,
                                                 int vert_end_idx,
                                                 ImVec2 pivot_in,
                                                 float cos_a,
    ...
    // Debug Tools
    CIMGUI_API void ImGui_DebugAllocHook(ImGuiDebugAllocInfo *info,
                                         int frame_count,
                                         void *ptr,
    ...
    CIMGUI_API void ImGui_DebugLocateItemOnHover(
    ...
    CIMGUI_API void
    ImGui_DebugHookIdInfo(ImGuiID id, ImGuiDataType data_type, const void *data_id, const void *data_id_end);
    CIMGUI_API void ImGui_DebugNodeColumns(ImGuiOldColumns *columns);
    CIMGUI_API void ImGui_DebugNodeDrawList(ImGuiWindow *window,
                                            ImGuiViewportP *viewport,
                                            const ImDrawList *draw_list,
                                            const char *label);
    CIMGUI_API void ImGui_DebugNodeDrawCmdShowMeshAndBoundingBox(ImDrawList *out_draw_list,
                                                                 const ImDrawList *draw_list,
    ...
    CIMGUI_API void ImGui_DebugNodeTextureEx(
        ImTextureData *tex,
        int int_id,
    ...
    CIMGUI_API void ImGui_DebugNodeWindowsListByBeginStackParent(ImGuiWindow **windows,
                                                                 int windows_size,
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    typedef ImFontLoader ImFontBuilderIO; // [renamed/changed in 1.92] The types are not actually compatible but we
...
#define ImFontAtlasRectId_GenerationShift_ (20)
    ...
    // We handle this with an indirection. While Rects[] may be in theory shuffled, compacted etc., RectsIndex[] cannot
    // it is keyed by ImFontAtlasRectId. RectsIndex[] is used both as an index into Rects[] and an index into itself.
    // This is basically a free-list. See ImFontAtlasBuildAllocRectIndexEntry() code. Having this also makes it easier
    ...
    struct ImVector_ImFontAtlasRectEntry_t
    {
        int Size;
        int Capacity;
...
// We avoid dragging imstb_rectpack.h into public header (partly because binding generators are having issues with it)
    ...
    struct ImVector_stbrp_node_im_t
    {
        int Size;
        int Capacity;
        stbrp_node_im *Data;
    ...
    struct ImFontAtlasBuilder_t
    {
        ...
        bool PreloadedAllGlyphsRanges; // Set when missing ImGuiBackendFlags_RendererHasTextures features forces atlas
    ...
    CIMGUI_API void cImFontAtlasBuildRenderBitmapFromString(
    ...
    CIMGUI_API void cImFontAtlasBuildGetOversampleFactors(ImFontConfig *src,
                                                          ImFontBaked *baked,
    ...
    CIMGUI_API bool cImFontAtlasFontInitOutput(ImFontAtlas *atlas,
    ...
    cImFontAtlasBakedGetOrAdd(ImFontAtlas *atlas, ImFont *font, float font_size, float font_rasterizer_density);
    CIMGUI_API ImFontBaked *
    cImFontAtlasBakedGetClosestMatch(ImFontAtlas *atlas, ImFont *font, float font_size, float font_rasterizer_density);
    CIMGUI_API ImFontBaked *cImFontAtlasBakedAdd(
        ImFontAtlas *atlas, ImFont *font, float font_size, float font_rasterizer_density, ImGuiID baked_id);
    CIMGUI_API void cImFontAtlasBakedDiscard(ImFontAtlas *atlas, ImFont *font, ImFontBaked *baked);
    CIMGUI_API ImFontGlyph *cImFontAtlasBakedAddFontGlyph(ImFontAtlas *atlas,
                                                          ImFontBaked *baked,
                                                          ImFontConfig *src,
                                                          const ImFontGlyph *in_glyph);
    CIMGUI_API void cImFontAtlasBakedAddFontGlyphAdvancedX(
        ImFontAtlas *atlas, ImFontBaked *baked, ImFontConfig *src, ImWchar codepoint, float advance_x);
    CIMGUI_API void
    cImFontAtlasBakedDiscardFontGlyph(ImFontAtlas *atlas, ImFont *font, ImFontBaked *baked, ImFontGlyph *glyph);
    CIMGUI_API void cImFontAtlasBakedSetFontGlyphBitmap(ImFontAtlas *atlas,
                                                        ImFontBaked *baked,
                                                        ImFontConfig *src,
                                                        ImFontGlyph *glyph,
                                                        ImTextureRect *r,
    ...
    CIMGUI_API ImFontAtlasRectId cImFontAtlasPackAddRect(ImFontAtlas *atlas,
                                                         int w,
                                                         int h); // Implied overwrite_entry = NULL
    CIMGUI_API ImFontAtlasRectId cImFontAtlasPackAddRectEx(ImFontAtlas *atlas,
                                                           int w,
                                                           int h,
    ...
    CIMGUI_API void cImFontAtlasTextureBlockConvert(const unsigned char *src_pixels,
                                                    ImTextureFormat src_fmt,
                                                    int src_pitch,
    ...
    CIMGUI_API void cImFontAtlasTextureBlockPostProcessMultiply(ImFontAtlasPostProcessData *data,
    ...
    CIMGUI_API void cImFontAtlasTextureBlockCopy(
        ImTextureData *src_tex, int src_x, int src_y, ImTextureData *dst_tex, int dst_x, int dst_y, int w, int h);
    CIMGUI_API void
    cImFontAtlasTextureBlockQueueUpload(ImFontAtlas *atlas, ImTextureData *tex, int x, int y, int w, int h);
    ...
    CIMGUI_API bool cImFontAtlasGetMouseCursorTexData(ImFontAtlas *atlas,
...
#ifdef IMGUI_ENABLE_TEST_ENGINE
    extern CIMGUI_API void cImGuiTestEngineHook_ItemAdd(ImGuiContext *ctx,
                                                        ImGuiID id,
                                                        ImRect bb,
                                                        const ImGuiLastItemData *item_data); // item_data may be NULL
    extern CIMGUI_API void
    cImGuiTestEngineHook_ItemInfo(ImGuiContext *ctx, ImGuiID id, const char *label, ImGuiItemStatusFlags flags);
    extern CIMGUI_API void cImGuiTestEngineHook_Log(ImGuiContext *ctx, const char *fmt, ...);
    extern CIMGUI_API const char *cImGuiTestEngine_FindItemDebugLabel(ImGuiContext *ctx, ImGuiID id);
